...
sint32 can_initServer(sint32 s32Baudrate, sint32 *s32CanFd)
{
	...
	sprintf(szCmd, "/root/ip link set can0 type can bitrate %d triple-sampling on", s32Baudrate);
	s32Ret = SAFE_System(szCmd, NORMAL_WAIT_TIME);
	if (SV_SUCCESS != s32Ret)
    {
        print_level(SV_ERROR, "cmd: %s failed.\n", szCmd);
...
void* can_Detect_Body(void *pvArg)
{
	sint16 sumTime;
	sint32 s32Ret, s32CanTxCount = 0, i = 0;
	static sint32 s_s32CanLastTxCount = -1;
	...
	while (pstCanInfo->bRunning)
	{
		...
		sumTime = (endTime.tv_sec - startTime.tv_sec) * 1000 + (endTime.tv_usec - startTime.tv_usec) / 1000;
		if (sumTime >= 15000)
		{
			strncpy(szCmd, "ifconfig | grep can0 -A3 | grep TX | awk -F ':' '{print$2}' | awk -F ' ' '{print$1}'", 128);
			s32Ret = SAFE_System_Recv(szCmd, szBuf, 64);
			if (SV_SUCCESS != s32Ret)
			{
			...
			if (strlen(szBuf) == 0)
			{
				print_level(SV_ERROR, "get can tx count failed.\n");
				goto exit;
			}

			s32CanTxCount = atoi(szBuf);
			if (s_s32CanLastTxCount != s32CanTxCount)
			{
				s_s32CanLastTxCount = s32CanTxCount;
			}
			else
			{
				print_level(SV_WARN, "can device need to restart, last: %d, now: %d\n", s_s32CanLastTxCount, s32CanTxCount);
                pthread_mutex_lock(&pstCanInfo->mutexSendData);
				SAFE_SV_System("canconfig can0 stop >> /dev/null");
				sleep_ms(50);
				SAFE_SV_System("canconfig can0 start >> /dev/null");
                pthread_mutex_unlock(&pstCanInfo->mutexSendData);
			}
...
sint32 CAN_Init(CFG_SYS_PARAM *pstSysParam)
{
	...
	s32Ret = can_initServer(pstSysParam->s32Baudrate*1000, &s32CanFd);
	if (SV_SUCCESS != s32Ret)
	{
		print_level(SV_ERROR, "can_initServer can0 error! baud: \n", pstSysParam->s32Baudrate);
		return SV_FAILURE;
	}
