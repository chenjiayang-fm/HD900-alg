...
sint32 mg_http_send(SV_BOOL bMg, struct mg_connection *nc, sint32 socketFd, uint8 *pu8Buf, sint32 s32Len)
{
    sint32 s32Ret;
    if (NULL == pu8Buf || 0 == s32Len || (bMg && NULL == nc))
    {
        return SV_FAILURE;
    }
    ...
    if (bMg)
	{        
		s32Ret = nc->is_tls ? mg_tls_send(nc, pu8Buf, s32Len) : mg_sock_send(nc, pu8Buf, s32Len);
    }
    else
    {
        s32Ret = send(socketFd, pu8Buf, s32Len, 0);
    }

    return s32Ret;
}

sint32 mg_http_recv(SV_BOOL bMg, struct mg_connection *nc, sint32 socketFd, uint8 *pu8Buf, sint32 s32Len)
{
    sint32 s32Ret;
    if (NULL == pu8Buf || 0 == s32Len || (bMg && NULL == nc))
    {
    ...
    if (bMg)
	{        
		s32Ret = nc->is_tls ? mg_tls_recv(nc, pu8Buf, s32Len) : mg_sock_recv(nc, pu8Buf, s32Len);
    }
    else
    {
        s32Ret = recv(socketFd, pu8Buf, s32Len, 0);
    }
...
static int uri_to_path2(struct mg_connection *c, struct mg_http_message *hm,
  ...
  int pathLen = 0;
  char *pszTmp = NULL;
  char szUri[MG_PATH_MAX] = {0};
  char szPath[MG_PATH_MAX] = {0};
  char *pszPasswd = MONGOOSE_FILE_PASSWD;
  int passwdLen = strlen(pszPasswd);
  
  mg_snprintf(szUri, MG_PATH_MAX, "%.*s", (int) hm->uri.len, hm->uri.ptr);
  pszTmp = strcasestr(szUri, pszPasswd);
  if (NULL != pszTmp)
  {
    strcpy(szPath, pszTmp+strlen(pszPasswd));
    pathLen = (int)hm->uri.len - strlen(pszPasswd);
  }
  else
  {
    strcpy(szPath, hm->uri.ptr);
    pathLen = (int)hm->uri.len;
  }
  ...
  mg_url_decode(szPath + url.len, pathLen - url.len, path + n,
                path_size - n, 0);
  path[path_size - 1] = '\0';  // Double-check
  remove_double_dots(path);
  n = strlen(path);
  MG_VERBOSE(("%lu %.*s -> %s", c->id, pathLen, szPath, path));
  while (n > 0 && path[n - 1] == '/') path[--n] = 0;  // Trim trailing slashes
  flags = mg_vcmp(&hm->uri, "/") == 0 ? MG_FS_DIR : fs->st(path, NULL, NULL);
  if (flags == 0) {
    mg_http_reply(c, 404, "", "Not found\n");  // Does not exist, doh
  } else if ((flags & MG_FS_DIR) && pathLen > 0 &&
             szPath[pathLen - 1] != '/') {
    mg_printf(c,
              "HTTP/1.1 301 Moved\r\n"
              "Location: %.*s/\r\n"
...
long mg_sock_send(struct mg_connection *c, const void *buf, size_t len) {
  long n;
  if (c->is_udp) {
    union usa usa;
    ...
    n = send(FD(c), (char *) buf, len, MSG_NONBLOCKING|MSG_NOSIGNAL);
...
long mg_sock_recv(struct mg_connection *c, void *buf, size_t len) {
  long n = 0;
  if (c->is_udp) {
    union usa usa;
    socklen_t slen = tousa(&c->rem, &usa);
...
static int mg_iotest(struct mg_mgr *mgr, int ms) {
#if MG_ARCH == MG_ARCH_FREERTOS_TCP
  struct mg_connection *c;
  for (c = mgr->conns; c != NULL; c = c->next) {
...
void mg_mgr_poll(struct mg_mgr *mgr, int ms) {
  ...
  if(ret == 0)
  {
      ...
      if (c->is_draining && c->send.len == 0) c->is_closing = 1;
      if (c->is_closing && c->label[0]!='J') close_conn(c);
    }
  }
}
#endif
...
size_t mkhdr(size_t len, int op, bool is_client, uint8_t *buf) {
  size_t n = 0;
