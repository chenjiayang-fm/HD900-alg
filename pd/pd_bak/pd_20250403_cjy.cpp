...
#define PD_MODEL_FLOWNET      "/root/model/flownet_pre.rknn" 
...
/* PD算法驱动线程 */
opticalflowanalyzeralg::COpticalFlowAnalyzer *cFlowAnalyzer = NULL;
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
   ...
   // Area_Info_init();
    ...
    stModelsPath.pModelCAD = modelpath;    // 光流模型
    
    opticalflowanalyzeralg::STOpticalFlowResult STFlowResult;  //实例化结构体用来存储光流结果

    // 设定车辆类型和相机安装方式
    opticalflowanalyzeralg::EVehicleTypeCode vehicletype = opticalflowanalyzeralg::E_VEHICLE_TYPE_NORMAL;
    opticalflowanalyzeralg::ECameraMountCode cameraMount = opticalflowanalyzeralg::E_CAMERA_MOUNT_REAR;
    
    //类初始化，图像高和宽，模型路径，车辆类型和相机安装位置,8为光流点间隔
    cFlowAnalyzer = new opticalflowanalyzeralg::COpticalFlowAnalyzer(352, 608, stModelsPath, vehicletype, cameraMount, 8, TRUE);
