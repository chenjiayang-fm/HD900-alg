...
#define RINGBUF_COUNT       3
#define RINGBUF_SIZE       642048 //PD_IMAGE_WIDTH *  PD_IMAGE_HEIGHT *3 //   RGB //642048 //
//static uint8_t *ringbuf[RINGBUF_COUNT] = {0};
TRWRingQueue<int> ringque(RINGBUF_COUNT-1);


#define RINGBUF_VDEC_COUNT       12
#define RINGBUF_VDEC_SIZE       307200 //PD_IMAGE_WIDTH *  PD_IMAGE_HEIGHT *3 //   RGB //642048 //
  ...
  //  Shared_memory->video_info.source_ID = *(yuv+32);
    ...
    for (int i = 0; i<480; i += 1) // 行数
    {
       ...
       continue;   
           ...
           vir_addr_dst[1] = *(yuv+38); //第0位字节储存source_id(通道号)
       //    printf("source_id:%d\n",vir_addr_dst[0]);
}
...
unsigned char dstData[3][RINGBUF_SIZE];
unsigned char MptrData[60][RINGBUF_VDEC_SIZE];
int s32time_out = 0;
...
sint32 mpp_vpss_GetAlgFrame()
{
 ...
 //   uint32_t cnt = 0;
    ...
    if(frame_save_begin)
    {
        ...
        fwrite(MptrData[index], 1, RINGBUF_VDEC_SIZE, file0);
        ...
        { 
           if(Index_temp == index) //开始，两者都为0
           {
               printf("_____start______ \n");
                       // *** write resource
           ...
           else if( ( MptrData[index][7] - MptrData[Index_temp][7] != 1 ) && ((MptrData[Index_temp][7] != 30) && (MptrData[index][7] != 0)) )
           {
               print_level(SV_ERROR, "frame sequence err now:%d,last:%d!\n", MptrData[index][7] , MptrData[Index_temp][7]);
               frame_sequence = 1;
           }
           else if ( (MptrData[Index_temp][7] == 30) && (MptrData[index][7] != 0) )
           {
               print_level(SV_ERROR, "frame sequence err now:%d,last:%d!\n", MptrData[index][7] , MptrData[Index_temp][7]);
               frame_sequence = 1;
           }
    ...
    // if(*(m_stVpssAlgFrame.mptr[s32BufIndex] +6) == 0)
    //     printf("** vdec write[%d] I:%d  \n",index, *(m_stVpssAlgFrame.mptr[s32BufIndex] +6));
    // else
    //     printf("** vdec write[%d] P:%d  \n",index, *(m_stVpssAlgFrame.mptr[s32BufIndex] +6));    

#if 0    //从解码到送进队列 ： 10ms
   Getframe_CH = *(m_stVpssAlgFrame.mptr[s32BufIndex] +32); 
      //  printf("******RK_MPI_MB_CreateBuffer*******\n");
       MEDIA_BUFFER m_mb = RK_MPI_MB_CreateBuffer(readbuff.length - 48, RK_TRUE, 0);  //yuv 420
        if(!m_mb)
        {
          print_level(SV_ERROR, "RK_MPI_MB_CreateBuffer failed\n");
        }

        memcpy(RK_MPI_MB_GetPtr(m_mb), m_stVpssAlgFrame.mptr[s32BufIndex] + 48, readbuff.length - 48);

     //   printf("******RK_MPI_MB_GetPtr*******\n");
        RK_MPI_MB_SetSize(m_mb, readbuff.length - 48);
      //  printf("#Send packet(%p, %zuBytes) to VDEC[0].\n", RK_MPI_MB_GetPtr(m_mb),RK_MPI_MB_GetSize(m_mb));

     //    printf("******RK_MPI_SYS_SendMediaBuffer*******\n");
        s32Ret =  RK_MPI_SYS_SendMediaBuffer(RK_ID_VDEC, Getframe_CH , m_mb);
      ...
      //  printf("******RK_MPI_SYS_GetMediaBuffer*******\n");
        //MEDIA_BUFFER pmb = RK_MPI_SYS_GetMediaBuffer(RK_ID_VDEC,0, 5000);
        //uint32_t pmb_size = RK_MPI_MB_GetSize(pmb);
        //void *pmb_addr = RK_MPI_MB_GetPtr(pmb);

        MEDIA_BUFFER RGA_m_mb = RK_MPI_SYS_GetMediaBuffer(RK_ID_RGA, Getframe_CH, 500); //解码数据直接送到RGA
            if(!RGA_m_mb )
            {
                Reset_frame[Getframe_CH] = 1;
                RK_MPI_MB_ReleaseBuffer(RGA_m_mb);
                print_level(SV_ERROR, "***RK_MPI_SYS_GetMediaBuffer%d failed.***\n",Getframe_CH);
                continue;
            }
    #if 0 //debug 
        MB_IMAGE_INFO_S stImageInfo = {0};
          s32Ret = RK_MPI_MB_GetImageInfo(RGA_m_mb, &stImageInfo);
        if (s32Ret)
          printf("Warn: Get image info failed! ret = %d\n", ret);

        printf("Get Frame:ptr:%p, fd:%d, size:%zu, mode:%d, channel:%d, "
               "timestamp:%lld, ImgInfo:<wxh %dx%d, fmt 0x%x>\n",
               RK_MPI_MB_GetPtr(RGA_m_mb), RK_MPI_MB_GetFD(RGA_m_mb), RK_MPI_MB_GetSize(RGA_m_mb),
               RK_MPI_MB_GetModeID(RGA_m_mb), RK_MPI_MB_GetChannelID(RGA_m_mb),
               RK_MPI_MB_GetTimestamp(RGA_m_mb), stImageInfo.u32Width,
               stImageInfo.u32Height, stImageInfo.enImgType);

            remove("./test.rgb");
            FILE *fp = fopen("./test.rgb", "wb+");
            fwrite(RK_MPI_MB_GetPtr(RGA_m_mb), 1, RK_MPI_MB_GetSize(RGA_m_mb), fp);
            fclose(fp);
    #endif
        //usleep(10 * 1000);
...
sint32 mpp_vpss_Vdec_test()
{
    ...
    // uint8_t Reset_Frame[4] = {0};
    // uint8_t rebuild_vdec[4] = {0};

    FILE * file  = fopen("/root/test264","r+");
    for(uint8_t i = 0; i < 60; i++)
    {
        fread(MptrData[i],1,RINGBUF_VDEC_SIZE,file);
    ...
    int Re_sendIframe[4] = {0};
    int Re_sendIframe_cnt[4] = {0};
    ...
    while(1)   
    {
       ...
       // printf("****delay ms__dec pic:%d ***** \n",u32StepTimeMs);
        ...
        Re_sendIframe[Getframe_CH] = rand() % 30;   

        vdec_cnt[Getframe_CH]++;

        if(Re_sendIframe[Getframe_CH] == 0)
        {
            index_vdec[Getframe_CH] = 0;
            Re_sendIframe_cnt[Getframe_CH] ++;
        }


        if(index_vdec[Getframe_CH] > 59)
        index_vdec[Getframe_CH] = 0;

        //printf("&& vdec read[%d] :%d,%d \n",index_vdec, *(MptrData[index_vdec] +6),*(MptrData[index_vdec] +7));
    ...
    //Getframe_CH = *(MptrData[index_vdec] +32); 
        ...
        //printf("__CH[%d] %d, frame:%d  \n",Getframe_CH, *(m_stVpssAlgFrame.mptr[index_vdec] +7) ,*(m_stVpssAlgFrame.mptr[index_vdec] +6));

      //  printf("******RK_MPI_MB_CreateBuffer*******\n");
       MEDIA_BUFFER m_mb = RK_MPI_MB_CreateBuffer(RINGBUF_VDEC_SIZE , RK_TRUE, 0);  //yuv 420
        if(!m_mb)
        {
          print_level(SV_ERROR, "RK_MPI_MB_CreateBuffer failed\n");
        }

        memcpy(RK_MPI_MB_GetPtr(m_mb), MptrData[index_vdec[Getframe_CH]] , RINGBUF_VDEC_SIZE );
     ...
     //    printf("******RK_MPI_MB_ReleaseBuffer*******\n");
        ...
        u32sum += u32StepTimeMs;
        u32cnt ++;
        if(u32cnt >100) //100*50ms = 5s
        {

           printf("100 run per : %d\n",u32sum / u32cnt );
           printf("0[%d][%d],1[%d][%d],2[%d][%d],3[%d][%d]\n",vdec_cnt[0],Re_sendIframe_cnt[0],vdec_cnt[1],Re_sendIframe_cnt[1],vdec_cnt[2],Re_sendIframe_cnt[2],vdec_cnt[3],Re_sendIframe_cnt[3]);
           vdec_cnt[0] = 0;
           vdec_cnt[1] = 0;
           vdec_cnt[2] = 0;
           vdec_cnt[3] = 0;

           Re_sendIframe_cnt[0] = 0;
           Re_sendIframe_cnt[1] = 0;
           Re_sendIframe_cnt[2] = 0;
           Re_sendIframe_cnt[3] = 0;
...
sint32 mpp_vpss_Vdec()
{
    ...
    while(1)   
    {
       ...
       // printf("****delay ms__dec pic:%d ***** \n",u32StepTimeMs);
        ...
        //printf("&& vdec read[%d] :%d trans:%d\n",index_vdec, *(MptrData[index_vdec] +7), *(MptrData[index_vdec] +8));
...
#if 1
uint32_t serial_init(const sint8* port, uint32_t baud)
{
    ...
    //    }
      ...
      //  memcpy( apvBuf00[index] , dstData[index], u32Width*u32Height*3);
       ...
       // SOURCE_ID =  *((uint8 *)apvBuf[s32CurChn]) ; //帧的第一个字节是source ID   
        SOURCE_ID = dstData[index][0]; 
       // DETECT_AREA_EN = dstData[index][1]; 
...
skip_alg:  
        ...
        { 
            ...
            /* 添加到串口数据中 */
            stAlgResult[u8LenAlgData].x1 = (short)(stPdResult.stResults[i].fX1 * 1024);
            stAlgResult[u8LenAlgData].y1 = (short)(stPdResult.stResults[i].fY1 * 600);
            stAlgResult[u8LenAlgData].x2 = (short)(stPdResult.stResults[i].fX2 * 1024);
            stAlgResult[u8LenAlgData].y2 = (short)(stPdResult.stResults[i].fY2 * 600);
...
//串口组包
       ...
       // s32WriteLen = write(m_stPdInfo.s32SerialFd, u8SerialData, sizeof(Algo_Result) * u8LenAlgData + 5);
        s32WriteLen = write(m_stPdInfo.s32SerialFd, u8SerialData, sizeof(Algo_Result) * u8LenAlgData + 6);
        ...
        // for(int i = 0; i< (sizeof(Algo_Result) * u8LenAlgData + 5) ;i++)
        // {
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
   ...
   s32Ret = mpp_vpss_InitAlgChn();   //打开264的句柄  m_stVpssAlgFrame.s32AlgChnFd
    ...
    s32Ret = pthread_create(&u32Tid_test, &attr, mpp_vpss_Vdec_test,NULL); //264解码
...
// #endif
    ...
    s32Ret = serial_init(ps8Port, 115200);
