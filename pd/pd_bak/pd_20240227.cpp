    ...
    else
    {
        if(frame_sequence)
        {
            if( !(frame_sequence_detect(m_stVpssAlgFrame.mptr[s32BufIndex][7],m_stVpssAlgFrame.mptr[Index_temp][7]))) //帧序号倒过来后仍然不对
            {
                ...
                Reset_Frame_done[Getframe_CH] = 0; //重发I帧
        ...
        if(FD_ISSET(m_stPdInfo.s32SerialFd[0],&rfds))
            {
                ...
                {
                    u16Crc = crc16_Gen(&buf[2], rxlen-2);
                    if(u16Crc == buf[rxlen-1] + buf[rxlen-2]<<8)
                    {
                        //  Area_Info_Get[buf[2]].line_top_x1     = buf ;
                        //  Area_Info_Get[buf[2]].line_top_x2     = buf ;
                        //  Area_Info_Get[buf[2]].line_bottom_x1  = buf ;
                        //  Area_Info_Get[buf[2]].line_bottom_x2  = buf ;
                        //  Area_Info_Get[buf[2]].line_top_y      = buf ;
                        //  Area_Info_Get[buf[2]].line_second_y   = buf ;
                        //  Area_Info_Get[buf[2]].line_third_y    = buf ;
                        //  Area_Info_Get[buf[2]].line_bottom_y   = buf ;
                            get_line_info(&Area_Info_Get[buf[2]]);

                            printf("recieve new area info\n");

                            unsigned char buf_temp[3] = {0};
                             buf_temp[0] = 0xff;
                             buf_temp[1] = 0xaa;
                             buf_temp[2] = buf[2];
                             write(m_stPdInfo.s32SerialFd[0],buf_temp[2], 1);  
                    }
                }
...
skip_alg:  
        ...
        { 
           ...
           stAlgResult[u8LenAlgData].y2 = (short)(stResults_f.fy2 * 600);
            ...
            stAlgResult[u8LenAlgData].alarm_type = s32AlarmMode;
