...
//  uint8_t skip_frame = 10; //初始的帧会不连续，需要跳过前几帧
    ...
    #if 0
        if(MptrData[index][21]) //切到单通道变成720p
        {
    ...
    if(frame_skip != 0)
    {
       int rel = ringque_VDEC.Write(s32BufIndex, [](int &release){     
    ...
    else
    {
        if(frame_sequence)
        {
            ...
            int rel1 = ringque_VDEC.Write(s32BufIndex, [](int &release){     
...
sint32 mpp_vpss_Vdec()
{
    ...
    else
    {
     ...
     //    printf("******RK_MPI_MB_ReleaseBuffer*******\n");
            int index = (counter % (RINGBUF_COUNT/4)) + (Getframe_CH * (4-1));  //每3个buf放一个通道的帧，溢出就丢
