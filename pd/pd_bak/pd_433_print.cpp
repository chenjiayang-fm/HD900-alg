...
void uart_send(sint32 fd, char * data)
{
    uint8_t i = 0;
    ...
    write(m_stPdInfo.s32SerialFd[1], data, i);   
...
#define RINGBUF_SIZE       PD_IMAGE_WIDTH*PD_IMAGE_HEIGHT*3 //PD_IMAGE_WIDTH *  PD_IMAGE_HEIGHT *3 //   RGB //642048 //
...
{   
    ...
    if(ret < 0)
    {
        ...
        sprintf(buf_test, "set format fail-");
        uart_send(m_stPdInfo.s32SerialFd[1], buf_test);
    ...
    for(int i=0; i<UVC_BUF_NUM; i++)
    {
        ...
        sprintf(buf_test, "requrey memory fail-");
        uart_send(m_stPdInfo.s32SerialFd[1], buf_test);
        memset(buf_test,0,sizeof(buf_test));
    ...
    if (u32BufNum <= 0)
    {
        ...
        sprintf(buf_test, "v4l2 mmap failed.-");
        uart_send(m_stPdInfo.s32SerialFd[1], buf_test);
        memset(buf_test,0,sizeof(buf_test));
        return SV_FAILURE;
    }
...
//  uint8_t skip_frame = 10; //初始的帧会不连续，需要跳过前几帧
    ...
    //printf("___%d___\n",(*(m_stVpssAlgFrame.mptr[s32BufIndex] +34)) );   
        ...
        //printf("[%d][%d]\n",MptrData[index][6],MptrData[index][7]);
        ...
        //    else if( ( MptrData[index][7] - MptrData[Index_temp][7] != 1 ) && ((MptrData[Index_temp][7] != 30) && (MptrData[index][7] != 0)) && ((MptrData[Index_temp][7] != 0) && (MptrData[index][7] != 30)) )
        //    {
        //        print_level(SV_ERROR, "frame sequence err now[%d]:%d,last[%d]:%d!\n", index,MptrData[index][7] , Index_temp,MptrData[Index_temp][7]);
        //        frame_sequence = 1;
        //    }
            else if( ( MptrData[index][7] - MptrData[Index_temp][7] != 1 )  )
           {
               print_level(SV_ERROR, "frame sequence err now[%d]:%d,last[%d]:%d!\n", index,MptrData[index][7] , Index_temp,MptrData[Index_temp][7]);
               frame_sequence = 1;
           }
        ...
        MEDIA_BUFFER RGA_m_mb = RK_MPI_SYS_GetMediaBuffer(RK_ID_RGA, VDEC_CH, 500); //解码数据直接送到RGA
            if(!RGA_m_mb )
            {
                 ...
                 print_level(SV_INFO, "***err:RK_MPI_SYS_GetMediaBuffer%d failed.***\n",VDEC_CH);
                        sprintf(buf_test, "err:RK_MPI_SYS_GetMediaBuffer%d failed-",VDEC_CH);
                        uart_send(m_stPdInfo.s32SerialFd[1], buf_test);
                        memset(buf_test,0,sizeof(buf_test)); 
               //  system("touch /root/err_exit");
                // mpp_vpss_Deinitialize();
...
sint32 mpp_vpss_Monitor()
{
    ...
    {
    s32time_out ++;
    if(s32time_out > 50) //5s没有进入算法，则重启
    {
    ...
    if(cnt ++ > 30)
    {
        SAFE_System_Recv("cat /sys/class/thermal/thermal_zone0/temp",buf,sizeof(buf));

        //printf("temperature:%s",buf);

        sprintf(buf_test, "temperature:%s--",buf);
        uart_send(m_stPdInfo.s32SerialFd[1], buf_test);
        memset(buf_test,0,sizeof(buf_test));
        ...
        if(FD_ISSET(m_stPdInfo.s32SerialFd[0],&rfds))
            {
               ...
               if(rxlen > 0)
               {
                ...
                //     /*启动upgrade.sh*/   //替换程序/库，添加新的文件，对文件进行备份BAK

                //     /*重新启动算法，发送更新成功*/

                //     /*如果哪个库或者应用程序有问题，导致算法没有跑起来，就跑upgrade_bak.sh*/


                // }
                // else
                // {
                        write(m_stPdInfo.s32SerialFd[1], buf, rxlen);  //控制433
                // }
               }
           }
       }
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
    ...
    sint8 *ps8Port1 = "/dev/ttyS3"; //433
    s32Ret = serial_init(ps8Port1, 115200,1);

   temp_SerialFd[0] = m_stPdInfo.s32SerialFd[0];

   temp_SerialFd[1] = m_stPdInfo.s32SerialFd[1];
   ...
   s32Ret = mpp_vpss_InitAlgChn();   //打开264的句柄  m_stVpssAlgFrame.s32AlgChnFd
    ...
    if (SV_SUCCESS != s32Ret)
    {
        ...
        sprintf(buf_test, "video node open failed--");
        uart_send(m_stPdInfo.s32SerialFd[1], buf_test);
        memset(buf_test,0,sizeof(buf_test));
    ...
    {
        sprintf(buf_test, "mpp_vpss_InitAlgChn success--");
        uart_send(m_stPdInfo.s32SerialFd[1], buf_test);
        memset(buf_test,0,sizeof(buf_test));   
    ...
    s32Ret = pthread_create(&u32Tid_monitor, &attr, mpp_vpss_Monitor,NULL);
        ...
        sprintf(buf_test, "alg init success--");
        uart_send(m_stPdInfo.s32SerialFd[1], buf_test);
        memset(buf_test,0,sizeof(buf_test));   
...
// #endif
    ...
    m_stPdInfo.s32SerialFd[0] =  temp_SerialFd[0];

    m_stPdInfo.s32SerialFd[1] =  temp_SerialFd[1];
