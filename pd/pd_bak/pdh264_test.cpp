...
#define version "1126-00-V1.0-20240705"
...
unsigned char MptrData_test[15][RINGBUF_VDEC_SIZE];
int s32time_out = 0;
...
//  uint8_t skip_frame = 10; //初始的帧会不连续，需要跳过前几帧
    ...
    FILE * file0 = fopen("/root/test264","w+");
    uint8_t frame_save_begin = 0;
    uint8_t frame_cnt = 0;
    ...
    if(frame_save_begin)
    {
        if(frame_cnt <= 15 )
        {
        printf("** vdec write[%d] %d,%d  \n",frame_cnt, *(m_stVpssAlgFrame.mptr[s32BufIndex] +6),*(m_stVpssAlgFrame.mptr[s32BufIndex] +7));    
        fwrite(m_stVpssAlgFrame.mptr[s32BufIndex], 1, RINGBUF_VDEC_SIZE, file0);
        fseek(file0,RINGBUF_VDEC_SIZE,SEEK_CUR);
        }
        else
        {
        frame_save_begin = 0;
...
sint32 mpp_vpss_FrameSave()
{
    ...
    FILE * file  = fopen("/root/test264","r+");

    unsigned char index_test = 0;
    for(uint8_t i = 0; i < 15; i++)
    {
        fread(MptrData_test[i],1,RINGBUF_VDEC_SIZE,file);
        fseek(file,RINGBUF_VDEC_SIZE,SEEK_CUR);
    }
    fclose(file);
    ...
    else if(H264_RESOL == RESOL_720P)
    {
       ...
       memcpy(MiddleData[index],MptrData_test[index_test],RINGBUF_VDEC_SIZE);
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
   ...
   // Area_Info_init();
    ...
    //s32Ret = pthread_create(&u32Tid_v4l2, &attr, mpp_vpss_GetAlgFrame,NULL);//跑这个来抓264的60帧
