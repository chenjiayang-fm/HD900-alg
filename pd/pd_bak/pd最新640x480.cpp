...
#define version "1126-00-V1.0-20240703"
...
void coordinate_remap(Algo_Result *stAlgResult_t,  Algo_Result_f stPdResult_t,uint8 Displaymode_t,uint32_t panel_width, uint32_t panel_height)
{
   ...
   switch (Displaymode_t)
   {
    ...
    {
        if(stAlgResult_t->x1 < 110 && stAlgResult_t->y2 <80 )
        {
            stAlgResult_t->x1 = 111;
            stAlgResult_t->y2 = 81;
        }
        
        if(stAlgResult_t->x1 > (panel_width-51) && stAlgResult_t->y2 < 80 )
        {
            stAlgResult_t->x1 = panel_width-50;
            stAlgResult_t->y2 = 81;
        }   
    ...
    {
        if(stAlgResult_t->x1 < (110+panel_width/2) && stAlgResult_t->y2 <80)
        {
            stAlgResult_t->x1 = (111+panel_width/2);
            stAlgResult_t->y2 = 51;
        }

        if(stAlgResult_t->x1 > (panel_width-52) && stAlgResult_t->y2 <50)
        {
            stAlgResult_t->x1 = (panel_width-53);
            stAlgResult_t->y2 = 51;
        }
    ...
    else
    {
        ...
        if(stAlgResult_t->x1 < 110 && stAlgResult_t->y2 < (50+panel_height/2))
        {
            stAlgResult_t->x1 = 111;
            stAlgResult_t->y2 = (51+panel_height/2);
        }
    }
    ...
    case DISPLAY_QUAD4:
        ...
        if(stAlgResult_t->x1 <  (110+panel_width/2) && stAlgResult_t->y1 < (50+panel_height/2))
        {
            stAlgResult_t->x1 = (111+panel_width/2);
            stAlgResult_t->y1 = (51+panel_height/2);
        }
...
//切换分割 模式
        ...
        else   //切到分割画面变成480p
        {
            if(H264_RESOL !=  RESOL_480P)
            {
                ...
                // if(save_cnt)
                // {
                //     sprintf(path, "/root/%dNORMAL.264",save_cnt);
                //     remove(path);
                //     FILE *fp = fopen(path, "wb+");
                //     if (NULL != fp)
                //     {   
                //         if(H264_RESOL == RESOL_480P)
                //             fwrite(m_stVpssAlgFrame.mptr[s32BufIndex], 1, RINGBUF_VDEC_SIZE, fp);
                //         else if(H264_RESOL == RESOL_720P)
                //             fwrite(m_stVpssAlgFrame.mptr[s32BufIndex], 1, RINGBUF_VDEC_SIZE_2, fp);
                //     }
                //     fclose(fp);
                //     save_cnt ++;
                // }
//通知重发I帧
    ...
    else
    {
        if(frame_sequence)
        {
            if( !(frame_sequence_detect(m_stVpssAlgFrame.mptr[s32BufIndex][7],m_stVpssAlgFrame.mptr[Index_temp][7]))) //帧序号倒过来后仍然不对
            {
                print_level(SV_ERROR, "frame sequence still err :%d ,%d",m_stVpssAlgFrame.mptr[s32BufIndex][7],m_stVpssAlgFrame.mptr[Index_temp][7] );
                for (uint8_t i = 0; i < 4; i++)
                {
                   Reset_Frame_done[i] = 0;
                }
            ...
            int rel1 = ringque_V4L2.Write(s32BufIndex, [](int &release){     
                    return ++release; }); //先发当前帧 s32BufIndex
            int rel2 = ringque_V4L2.Write(Index_temp, [](int &release){     
                    return ++release; }); //再发上一帧 Index_temp

            s32BufIndex = Index_temp;
            }
            frame_sequence = 0;
        }
        else if( !(frame_sequence_detect(m_stVpssAlgFrame.mptr[Index_temp][7],m_stVpssAlgFrame.mptr[s32BufIndex][7])))
           {
            print_level(SV_ERROR, "frame sequence err now:%d,last:%d\n", m_stVpssAlgFrame.mptr[s32BufIndex][7] ,m_stVpssAlgFrame.mptr[Index_temp][7]); //帧倒序
            frame_sequence = 1;       
           }
        else
        {
    ...
    Index_temp = s32BufIndex;  //记录上一个帧序号
#endif
...
sint32 mpp_vpss_FrameSave()
{
   ...
   // printf("vdec now %d,last %d\n",MiddleData[index][7],MiddleData[Index_temp][7]);
    ...
    //切换分割 模式
    #if 1
        if(! (*(MiddleData[index] +34)&0x80) ) //切到单通道变成720p
        {
            if(H264_RESOL !=  RESOL_720P)
            {
                H264_RESOL = RESOL_720P;
...
sint32 mpp_vdec_init()
{
    ...
    for(int i = 0; i<4 ; i++)
    {
        ...
        if(s32Ret != RK_SUCCESS)
        {
            ...
            stRgaAttr.stImgIn.u32Y = 30;  //上面裁剪30个像素
            stRgaAttr.stImgIn.imgType = IMAGE_TYPE_NV12;
            stRgaAttr.stImgIn.u32Width = u32Width;     //要裁剪的宽高
            stRgaAttr.stImgIn.u32Height = u32Height-80;  //下面裁剪80个像素
...
sint32 mpp_vpss_Vdec()
{
   ...
   // sint32 s32Fd = m_stVpssAlgFrame.s32AlgChnFd;
    ...
    if(! (*(MiddleData[index_vdec] +34)&0x80) )//== RESOL_720P
