...
#define version "1126-00-V1.1-20240913"
...
void coordinate_remap(Algo_Result *stAlgResult_t,  Algo_Result_f stPdResult_t,uint8 Displaymode_t,uint32_t panel_width, uint32_t panel_height)
{
    ...
    else
    {
        ...
        if(stAlgResult_t->x1 < 110 && stAlgResult_t->y1 <90 )
        {
            ...
            stAlgResult_t->y2 = stAlgResult_t->y2 + (90 - stAlgResult_t->y1);

            stAlgResult_t->x1 = 111;
            stAlgResult_t->y1 = 90;
        }
        
        if(stAlgResult_t->x2 > (panel_width-51) && stAlgResult_t->y2 < 90 )
        {
            stAlgResult_t->x1 = stAlgResult_t->x1 - (stAlgResult_t->x2-(panel_width-50));
            stAlgResult_t->y1 = stAlgResult_t->y1 - (stAlgResult_t->y2-90);
        ...
        //分割线  分割线宽度是4个像素
        if(stAlgResult_t->x1 <= 4)
        {
            stAlgResult_t->x2 = stAlgResult_t->x2 + (5 - stAlgResult_t->x1);
            stAlgResult_t->x1 = 5;
        } 

        if(stAlgResult_t->x2 >= (panel_width - 4) )
        {
            stAlgResult_t->x1 = stAlgResult_t->x1 - (stAlgResult_t->x2-(panel_width - 4));
            stAlgResult_t->x2 = (panel_width - 4);
        } 
        
        if(stAlgResult_t->y1 <= 4)
        {
            stAlgResult_t->y2 = stAlgResult_t->y2 + (5 - stAlgResult_t->y1);
            stAlgResult_t->y1 = 5;
        } 

        if(stAlgResult_t->y2 >= (panel_height - 4))
        {
            stAlgResult_t->y1 = stAlgResult_t->y1 - (stAlgResult_t->y2-(panel_height - 4));
            stAlgResult_t->y2 = (panel_height - 4);
        } 

            if (stAlgResult_t->x2 > (1024-20))
            {
            	stAlgResult_t->x2 = (1024-20);
            }
            if (stAlgResult_t->y2 > (600-20))
            {
            	stAlgResult_t->y2 = (600-20);
            }
   ...
   switch (Displaymode_t)
   {
    ...
    case DISPLAY_DOUBLE1:
        stAlgResult_t->x1 = (short)(stAlgResult_t->x1 /2);
        stAlgResult_t->y1 = (short)(stAlgResult_t->y1  );
        stAlgResult_t->x2 = (short)(stAlgResult_t->x2 /2);
        stAlgResult_t->y2 = (short)(stAlgResult_t->y2  );

    break;

    case DISPLAY_DOUBLE2:
        stAlgResult_t->x1 = (short)(stAlgResult_t->x1    /2 + panel_width/2);
        stAlgResult_t->y1 = (short)(stAlgResult_t->y1                );
        stAlgResult_t->x2 = (short)(stAlgResult_t->x2    /2 + panel_width/2);
        stAlgResult_t->y2 = (short)(stAlgResult_t->y2                );   
    ...
    case DISPLAY_QUAD1:
        stAlgResult_t->x1 = (short)(stAlgResult_t->x1 /2 );
        stAlgResult_t->y1 = (short)(stAlgResult_t->y1 /2);
        stAlgResult_t->x2 = (short)(stAlgResult_t->x2 /2);
        stAlgResult_t->y2 = (short)(stAlgResult_t->y2 /2);
    ...
    case DISPLAY_QUAD3:
        stAlgResult_t->x1 = (short)(stAlgResult_t->x1/2 );
        stAlgResult_t->y1 = (short)(stAlgResult_t->y1/2 + panel_height/2);
        stAlgResult_t->x2 = (short)(stAlgResult_t->x2/2 );
        stAlgResult_t->y2 = (short)(stAlgResult_t->y2/2 + panel_height/2);
    ...
    default:
        stAlgResult_t->x1 = (short)(stAlgResult_t->x1 * panel_width);
        stAlgResult_t->y1 = (short)(stAlgResult_t->y1 * panel_height);
        stAlgResult_t->x2 = (short)(stAlgResult_t->x2 * panel_width);
        stAlgResult_t->y2 = (short)(stAlgResult_t->y2 * panel_height);
    break;
   }
}
...
{   
    ...
    for(int i=0; i<UVC_BUF_NUM; i++)
    {
        ...
        //用户空间的程序就可以直接访问这些缓冲区，而不需要额外的数据复制
...
//  uint8_t skip_frame = 10; //初始的帧会不连续，需要跳过前几帧
    ...
    //     }
       ...
       s32Ret = ioctl(s32Fd, VIDIOC_DQBUF, &readbuff);//将已经填充好数据的缓冲区从内核队列中取出，供用户空间程序使用
       if(s32Ret < 0)
       {
    ...
    s32Ret = ioctl(s32Fd, VIDIOC_QBUF, &readbuff); //将一个空的缓冲区排队到内核队列，供视频设备填充数据
       if (s32Ret < 0)
       {
           print_level(SV_ERROR, "put fail!\n");
           return SV_FAILURE;
...
sint32 mpp_vpss_Vdec()
{
   ...
   // sint32 s32Fd = m_stVpssAlgFrame.s32AlgChnFd;
    ...
    MiddleData[index_vdec][9] = 0; //如果9,10两位都为1的话，解码会失败
    MiddleData[index_vdec][10] = 0;
    ...
    if(H264_RESOL == RESOL_480P)
    {
        ...
        memcpy(RK_MPI_MB_GetPtr(m_mb), (MiddleData[index_vdec]), (RINGBUF_VDEC_SIZE) );
    }
    else if(H264_RESOL == RESOL_720P)
    {
        ...
        memcpy(RK_MPI_MB_GetPtr(m_mb), (MiddleData[index_vdec]) , (RINGBUF_VDEC_SIZE_2) );    
    }
    else
    {
        m_mb = RK_MPI_MB_CreateBuffer(RINGBUF_VDEC_SIZE , RK_TRUE, 0); 
...
void pd_Alarm_Out_test(void *pvArg)
{
   // static uint8_t lock = 0;
    ...
    if(( lmode && TRIGGER_OUTPUT_EN[stAlarmNotice.s32Chn]))
        flag = 1;
    ...
    //    }
      ...
      //  memcpy( apvBuf00[index] , dstData[index], u32Width*u32Height*3);
       ...
       // SOURCE_ID =  *((uint8 *)apvBuf[s32CurChn]) ; //帧的第一个字节是source ID   
        ...
        TRIGGER_OUTPUT_EN[SOURCE_ID] = dstData[index][RINGBUF_SIZE-extern_num+4];
   ...
   // if(BsdRange_TYPE == 0)
    //{
        ...
        if(DisplayMode == DISPLAY_SINGLE){
            ...
            if((stAlgResult[u8LenAlgData].y2 - stAlgResult[u8LenAlgData].y1 < 30))
            {
                stAlgResult[u8LenAlgData].y2 = stAlgResult[u8LenAlgData].y1+30;
            }
        }
        else
        {
            if (((stAlgResult[u8LenAlgData].x2 - stAlgResult[u8LenAlgData].x1 )< 15) )
            {
                stAlgResult[u8LenAlgData].x2 = stAlgResult[u8LenAlgData].x1 + 15;
            }
