...
#define version "1126-00-V1.0-20240619"
...
sint32 mpp_vpss_FrameSave()
{
 ...
 return ++release; });//直接发 s32BufIndex 
    ...
    //         {
    //             print_level(SV_ERROR, "frame sequence still err :%d ,%d",MiddleData[index][7],MiddleData[Index_temp][7] );
    //             for (uint8_t i = 0; i < 4; i++)
    //             {
    //                Reset_Frame_done[i] = 0;
    //             }
            ...
            {     
                ...
                else
                {
                    ...
                    printf("crc: %x - %x\n", u16Crc,((buf[(buf[2]-1)]<< 8) + buf[buf[2]-2]));
                        ...
                        Area_Info_Get[ch].line_bottom_x2   =  (buf_temp[11]<<8)  +  (buf_temp[10] );
                        Area_Info_Get[ch].line_top_y       =  (buf_temp[13]<<8)  +  (buf_temp[12] );
                        // Area_Info_Get[ch].line_second_y    =  (buf_temp[15]<<8)  +  (buf_temp[14] );
                        // Area_Info_Get[ch].line_third_y     =  (buf_temp[17]<<8)  +  (buf_temp[16] );
                        Area_Info_Get[ch].line_bottom_y    =  (buf_temp[19]<<8)  +  (buf_temp[18] );
    ...
    //    }
      ...
      //  memcpy( apvBuf00[index] , dstData[index], u32Width*u32Height*3);
       ...
       // SOURCE_ID =  *((uint8 *)apvBuf[s32CurChn]) ; //帧的第一个字节是source ID   
        ...
        ALARM_OUTPUT_EN = dstData[index][RINGBUF_SIZE-extern_num+4];
