...
#define version "1126-00-V1.0-20240617"
...
sint32 pd_Alarm_Post(PD_ALARM_NOTICE_S stAlarmNotice)
{
   ...
   thpool_add_work(m_stPdInfo.thpool, pd_avAlarmer_Play, &stAlarmNoticeLocal);
    ...
    // if(stAlarmNotice.enMode != 0  &&  lock == 0 && ALARM_OUTPUT_EN)
    // {
    //     pd_Alarm_Out_Enable(stAlarmNotice.enMode);
    //     lock = 1;
    // }
    // else if(stAlarmNotice.enMode == 0 && lock == 1 )
    // {
    //     pd_Alarm_Out_Reset();
    //     lock = 0;
    // }
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
   ...
   s32Ret = mpp_vpss_InitAlgChn();   //打开264的句柄  m_stVpssAlgFrame.s32AlgChnFd
    ...
    char cmdStr[128] = {0};
    if(access("/root/AlgCntFile", F_OK) != 0) {    //test
        sprintf(cmdStr, "touch /root/AlgCntFile");
        SAFE_SV_System(cmdStr);
    } 
