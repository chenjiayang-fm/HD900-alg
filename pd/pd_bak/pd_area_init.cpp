...
sint32 area_detect(Algo_Result result , Area_Info AreaInfo)
{
    ...
    {  
        ...
        if (result.y1 <= AreaInfo.line_bottom_y && result.y1 >= AreaInfo.line_top_y ) //3.  y1 在梯形区域外部，y1在梯形内部的情况
         {
...
sint32 mpp_vpss_Monitor()
{
    ...
    {
    s32time_out ++;
    if(s32time_out > 20) //60s没有进入算法，则重启
    {
        ...
        if(FD_ISSET(m_stPdInfo.s32SerialFd[0],&rfds))
            ...
            {    
...
#if 0 
                if(buf_temp[0] == 0xff && buf_temp[1] == 0xbb)
                {
                    printf("buf:");   
                    for(int i = 0; i < (buf_temp[2]+2) ; i++)
                    {   
                       printf("%02x",buf_temp[i]);
                    } 
                    printf("--total:%d\n", (buf_temp[2]+2));

                    u16Crc = crc16_Gen(&buf_temp[2], buf_temp[2]-2);
                    printf("crc: %x - %x\n", u16Crc,((buf_temp[(buf_temp[2]+1)]<< 8) + buf_temp[buf_temp[2]]));

                        if(u16Crc == ((buf_temp[(buf_temp[2]+1)]<< 8) + buf_temp[buf_temp[2]]) ){  //crc校验通过
                        
                        Area_Info_Get[buf_temp[3]].line_top_x1     = buf_temp[4]  +  (buf_temp[5] <<8);
                        Area_Info_Get[buf_temp[3]].line_top_x2     = buf_temp[6]  +  (buf_temp[7] <<8);
                        Area_Info_Get[buf_temp[3]].line_bottom_x1  = buf_temp[8]  +  (buf_temp[9] <<8);
                        Area_Info_Get[buf_temp[3]].line_bottom_x2  = buf_temp[10] +  (buf_temp[11]<<8);
                        Area_Info_Get[buf_temp[3]].line_top_y      = buf_temp[12] +  (buf_temp[13]<<8);
                        Area_Info_Get[buf_temp[3]].line_second_y   = buf_temp[14] +  (buf_temp[15]<<8);
                        Area_Info_Get[buf_temp[3]].line_third_y    = buf_temp[16] +  (buf_temp[17]<<8);
                        Area_Info_Get[buf_temp[3]].line_bottom_y   = buf_temp[18] +  (buf_temp[19]<<8);
                        get_line_info(&Area_Info_Get[buf[3]]);
                        printf("recieve new area info[%d],x1:%d,x2:%d,x3:%d,x4:%d,y1:%d,y2:%d,y3:%d,y4:%d\n",buf[3], Area_Info_Get[buf_temp[3]].line_top_x1 ,Area_Info_Get[buf_temp[3]].line_top_x2 ,\
                ...
                if(flag == 0)
                {
                    if(buf_temp[0] == 0xff && buf_temp[1] == 0xbb)
                    {
                        ...
                        if(bufindex >= 22)//一次性全收完了                
                       {
                        ...
                        Area_Info_Get[buf_temp[3]].line_third_y    = buf_temp[16] +  (buf_temp[17]<<8);
                        Area_Info_Get[buf_temp[3]].line_bottom_y   = buf_temp[18] +  (buf_temp[19]<<8);
                        get_line_info(&Area_Info_Get[buf_temp[3]]);
                        //获得梯形的四个点的坐标
                        Area_Info_Get[buf_temp[3]].p[0].x  = Area_Info_Get[buf_temp[3]].line_top_x1;
                        Area_Info_Get[buf_temp[3]].p[0].y  = Area_Info_Get[buf_temp[3]].line_top_y;
                        Area_Info_Get[buf_temp[3]].p[1].x  = Area_Info_Get[buf_temp[3]].line_top_x2;
                        Area_Info_Get[buf_temp[3]].p[1].y  = Area_Info_Get[buf_temp[3]].line_top_y;
                        Area_Info_Get[buf_temp[3]].p[2].x  = Area_Info_Get[buf_temp[3]].line_bottom_x1;
                        Area_Info_Get[buf_temp[3]].p[2].y  = Area_Info_Get[buf_temp[3]].line_bottom_y;
                        Area_Info_Get[buf_temp[3]].p[3].x  = Area_Info_Get[buf_temp[3]].line_bottom_x2;
                        Area_Info_Get[buf_temp[3]].p[3].y  = Area_Info_Get[buf_temp[3]].line_bottom_y;
                ...
                else
                {
                    memcpy(&buf[bufindex],buf_temp,rxlen);
                    bufindex = bufindex + rxlen ;
                    if( bufindex  >= (buf[2]+2))  // 第2位是包长度  //收完了完整的包
                    {
                        //收完整个包开始处理
                    ...
                    u16Crc = crc16_Gen(&buf[2], buf[2]-2);
                    printf("crc: %x - %x\n", u16Crc,((buf[(buf[2]+1)]<< 8) + buf[buf[2]]));

                        if(u16Crc == ((buf[(buf[2]+1)]<< 8) + buf[buf[2]]) ){  //crc校验通过
                        
                        Area_Info_Get[buf[3]].line_top_x1     = buf[4]  +  (buf[5] <<8);
                        Area_Info_Get[buf[3]].line_top_x2     = buf[6]  +  (buf[7] <<8);
                        Area_Info_Get[buf[3]].line_bottom_x1  = buf[8]  +  (buf[9] <<8);
                        Area_Info_Get[buf[3]].line_bottom_x2  = buf[10] +  (buf[11]<<8);
                        Area_Info_Get[buf[3]].line_top_y      = buf[12] +  (buf[13]<<8);
                        Area_Info_Get[buf[3]].line_second_y   = buf[14] +  (buf[15]<<8);
                        Area_Info_Get[buf[3]].line_third_y    = buf[16] +  (buf[17]<<8);
                        Area_Info_Get[buf[3]].line_bottom_y   = buf[18] +  (buf[19]<<8);
                        get_line_info(&Area_Info_Get[buf[3]]);
                        //获得梯形的四个点的坐标
                        Area_Info_Get[buf[3]].p[0].x  = Area_Info_Get[buf[3]].line_top_x1;
                        Area_Info_Get[buf[3]].p[0].y  = Area_Info_Get[buf[3]].line_top_y;
                        Area_Info_Get[buf[3]].p[1].x  = Area_Info_Get[buf[3]].line_top_x2;
                        Area_Info_Get[buf[3]].p[1].y  = Area_Info_Get[buf[3]].line_top_y;
                        Area_Info_Get[buf[3]].p[2].x  = Area_Info_Get[buf[3]].line_bottom_x1;
                        Area_Info_Get[buf[3]].p[2].y  = Area_Info_Get[buf[3]].line_bottom_y;
                        Area_Info_Get[buf[3]].p[3].x  = Area_Info_Get[buf[3]].line_bottom_x2;
                        Area_Info_Get[buf[3]].p[3].y  = Area_Info_Get[buf[3]].line_bottom_y;
...
skip_alg:  
        ...
        { 
           ...
           s32AlarmMode = area_detect(stAlgResult[i],Area_Info_Get_init[SOURCE_ID]);
//            printf("alarm:%d\n",s32AlarmMode);
