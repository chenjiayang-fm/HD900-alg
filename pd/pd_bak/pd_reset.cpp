...
{   
    ...
    if(0 == access("/root/arg_720p" , F_OK))
    {
...
//  uint8_t skip_frame = 10; //初始的帧会不连续，需要跳过前几帧
    ...
    // printf("CH[%d]:[%d]-[%d]\n",MptrData[index][32],MptrData[index][7],MptrData[index][6]);
...
// #endif
        ...
        apcsPdsAlg->AlgResult(&stPdResult, pdsa32::E_POST_TRACK, pstPdInfo->stTrackParam, 0);

        // for (i = 0; i < stPdResult.u32Nums; i++)
        // {
        //     printf("type  %d\n",stPdResult.stResults[i].classes );
        // }
...
sh: can't create /sys/class/gpio_stonkam/RGB/rgbstate: nonexistent directory***********/
        ...
        printf("detect num[%d]:num:%d,p:%d,c:%d \n",SOURCE_ID ,stPdResult.u32Nums,  person_cnt,car_cnt);
