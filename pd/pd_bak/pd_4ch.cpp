...
#define RINGBUF_COUNT       4
#define RINGBUF_SIZE       642048 //PD_IMAGE_WIDTH *  PD_IMAGE_HEIGHT *3 //   RGB //642048 //

TRWRingQueue<int> ringque0(RINGBUF_COUNT-1); //四个通道分四个队列
TRWRingQueue<int> ringque1(RINGBUF_COUNT-1);
TRWRingQueue<int> ringque2(RINGBUF_COUNT-1);
TRWRingQueue<int> ringque3(RINGBUF_COUNT-1);
...
uint32_t TEST_RINGCOUNT0 = 0;
uint32_t TEST_RINGCOUNT1 = 0;
uint32_t TEST_RINGCOUNT2 = 0;
uint32_t TEST_RINGCOUNT3 = 0;
...
unsigned char dstData0[RINGBUF_COUNT][RINGBUF_SIZE];
unsigned char dstData1[RINGBUF_COUNT][RINGBUF_SIZE];
unsigned char dstData2[RINGBUF_COUNT][RINGBUF_SIZE];
unsigned char dstData3[RINGBUF_COUNT][RINGBUF_SIZE];
...
sint32 mpp_vpss_Vdec()
{
    ...
    RK_MPI_MB_SetSize(m_mb, RINGBUF_VDEC_SIZE );
     ...
     //    printf("******RK_MPI_MB_ReleaseBuffer*******\n");
                if( Getframe_CH == 0 )
                { 
                     index = counter0 % RINGBUF_COUNT;
                }
                else if(  Getframe_CH == 1 )
                {
                     index = counter1 % RINGBUF_COUNT;
                }
                else if(  Getframe_CH == 2 )
                {
                     index = counter2 % RINGBUF_COUNT;
                }
                else if(  Getframe_CH == 3 )
                {
                     index = counter3 % RINGBUF_COUNT;
                }

            //int index = counter0 % RINGBUF_COUNT;

                //memcpy(dstData[index],RK_MPI_MB_GetPtr(RGA_m_mb),RINGBUF_SIZE);
                if( Getframe_CH == 0 )
                { 
                     memcpy(dstData0[index],RK_MPI_MB_GetPtr(RGA_m_mb),RINGBUF_SIZE);
                }
                else if(  Getframe_CH == 1 )
                {
                     memcpy(dstData1[index],RK_MPI_MB_GetPtr(RGA_m_mb),RINGBUF_SIZE);
                }
                else if(  Getframe_CH == 2 )
                {
                     memcpy(dstData2[index],RK_MPI_MB_GetPtr(RGA_m_mb),RINGBUF_SIZE);
                }
                else if(  Getframe_CH == 3 )
                {
                     memcpy(dstData3[index],RK_MPI_MB_GetPtr(RGA_m_mb),RINGBUF_SIZE);
                }
        ...
        // }
            ...
            //dstData[index][0] =  Getframe_CH;
                ...
                if( Getframe_CH == 0 )
                { 
                    if(TEST_RINGCOUNT0 < RINGBUF_COUNT)
                    TEST_RINGCOUNT0 ++;
                    int rel = ringque0.Write(index, [](int &release){ 
                    return ++release; 
                     });
                ...
                {
                    if(TEST_RINGCOUNT1 < RINGBUF_COUNT)
                    TEST_RINGCOUNT1 ++;
                    int rel = ringque1.Write(index, [](int &release){ 
                    return ++release; 
                     });
                }
                else if(  Getframe_CH == 2 )
                {
                    if(TEST_RINGCOUNT2 < RINGBUF_COUNT)
                    TEST_RINGCOUNT2 ++;
                    int rel = ringque2.Write(index, [](int &release){ 
                    return ++release; 
                     });    
                }
                else if(  Getframe_CH == 3 )
                {
                    if(TEST_RINGCOUNT3 < RINGBUF_COUNT)
                    TEST_RINGCOUNT3 ++;
                    int rel = ringque3.Write(index, [](int &release){ 
                    return ++release; 
                     });
                }
        ...
        if(FD_ISSET(m_stPdInfo.s32SerialFd[0],&rfds))
            {
               ...
               if(rxlen > 0)
               {
                 ...
                 write(m_stPdInfo.s32SerialFd[1], buf, rxlen);
               }
           }
       }
    }
...
// #endif
    ...
    if(u8lastalgCH == 3  && TEST_RINGCOUNT0 != 0)
    {
        if(TEST_RINGCOUNT0 > 0)
        TEST_RINGCOUNT0--;
        u8lastalgCH = 0;
        index = ringque0.Read(chn, timeout, [/*UtilizeResource*/&ringque0](int &current){
            //cout << "    --- reader" << " --- " << GetMtimestamp() << " rpos:" << ringque.rpos_[0] << " index:" << current << " value:" << *ringbuf[current] << endl;
            //printf("    --- reader --- %llu, rpos: %d, index: %d, value: %#x\n", GetMtimestamp(), ringque.rpos_[0], current, (uint32_t)(*ringbuf[current]));
            // usleep(10000); // 10ms
             //sleep_ms(10);
    ...
    else if(u8lastalgCH == 0 && TEST_RINGCOUNT1 != 0)
    {
        if(TEST_RINGCOUNT0 > 0)
        TEST_RINGCOUNT1--;
        u8lastalgCH = 1; 
        index = ringque1.Read(chn, timeout, [/*UtilizeResource*/&ringque1](int &current){
            //cout << "    --- reader" << " --- " << GetMtimestamp() << " rpos:" << ringque.rpos_[0] << " index:" << current << " value:" << *ringbuf[current] << endl;
            //printf("    --- reader --- %llu, rpos: %d, index: %d, value: %#x\n", GetMtimestamp(), ringque.rpos_[0], current, (uint32_t)(*ringbuf[current]));
            // usleep(10000); // 10ms
             //sleep_ms(10);
    ...
    else if(u8lastalgCH == 1 && TEST_RINGCOUNT2 != 0)
    {
        if(TEST_RINGCOUNT2 > 0)
        TEST_RINGCOUNT2--;
        u8lastalgCH = 2;
        index = ringque2.Read(chn, timeout, [/*UtilizeResource*/&ringque2](int &current){
            //cout << "    --- reader" << " --- " << GetMtimestamp() << " rpos:" << ringque.rpos_[0] << " index:" << current << " value:" << *ringbuf[current] << endl;
            //printf("    --- reader --- %llu, rpos: %d, index: %d, value: %#x\n", GetMtimestamp(), ringque.rpos_[0], current, (uint32_t)(*ringbuf[current]));
            // usleep(10000); // 10ms
             //sleep_ms(10);
    ...
    else if(u8lastalgCH == 2 && TEST_RINGCOUNT3 != 0)
    {
        if(TEST_RINGCOUNT3 > 0)
        TEST_RINGCOUNT3--;
        u8lastalgCH = 3;
        index = ringque3.Read(chn, timeout, [/*UtilizeResource*/&ringque3](int &current){
            //cout << "    --- reader" << " --- " << GetMtimestamp() << " rpos:" << ringque.rpos_[0] << " index:" << current << " value:" << *ringbuf[current] << endl;
            //printf("    --- reader --- %llu, rpos: %d, index: %d, value: %#x\n", GetMtimestamp(), ringque.rpos_[0], current, (uint32_t)(*ringbuf[current]));
            // usleep(10000); // 10ms
             //sleep_ms(10);
      ...
      //  memcpy( apvBuf00[index] , dstData[index], u32Width*u32Height*3);
       ...
       // SOURCE_ID =  *((uint8 *)apvBuf[s32CurChn]) ; //帧的第一个字节是source ID   
        SOURCE_ID = u8lastalgCH; 


       //testcnt[SOURCE_ID]++;
       // DETECT_AREA_EN = dstData[index][1]; 
    if(SOURCE_ID == 0)
        s32Ret = apcsPdsAlg->AlgForward(dstData0[index] , u32Width, u32Height, 0, 0);
    else if(SOURCE_ID == 1)
        s32Ret = apcsPdsAlg->AlgForward(dstData1[index] , u32Width, u32Height, 0, 0);
    else if(SOURCE_ID == 2)
        s32Ret = apcsPdsAlg->AlgForward(dstData2[index] , u32Width, u32Height, 0, 0);
    else if(SOURCE_ID == 3)
        s32Ret = apcsPdsAlg->AlgForward(dstData3[index] , u32Width, u32Height, 0, 0);
