...
sint32 mpp_vpss_Vdec()
{
    ...
    while(1)   
    {
       ...
       // printf("****delay ms__dec pic:%d ***** \n",u32StepTimeMs);
        ...
        index_vdec = ringque_V4L2.Read(chn, timeout, [/*UtilizeResource*/&ringque_VDEC](int &current){
            return current; //0;
        });
    ...
    Getframe_CH = *(m_stVpssAlgFrame.mptr[index_vdec] + 32); 
    Detect_en_flag = *(m_stVpssAlgFrame.mptr[index_vdec] + 38);
    DisplayMode_flag = *(m_stVpssAlgFrame.mptr[index_vdec] + 8);
    DetectMode_flag = *(m_stVpssAlgFrame.mptr[index_vdec] + 9);
    Alarm_en_flag = *(m_stVpssAlgFrame.mptr[index_vdec] + 10);
    BsdRange_type_flag = *(m_stVpssAlgFrame.mptr[index_vdec] + 11);


    if(! (*(m_stVpssAlgFrame.mptr[index_vdec] +34)&0x80) )//== RESOL_720P
        VDEC_CH = Getframe_CH + 4;
    else
        VDEC_CH = Getframe_CH ; 

       // printf("vdec read[%d] :%d \n",Getframe_CH,u32StepTimeMs);
    ...
    if(H264_RESOL == RESOL_480P)
    {
        ...
        memcpy(RK_MPI_MB_GetPtr(m_mb), m_stVpssAlgFrame.mptr[index_vdec] , RINGBUF_VDEC_SIZE );
    }
    else if(H264_RESOL == RESOL_720P)
    {
        ...
        memcpy(RK_MPI_MB_GetPtr(m_mb), m_stVpssAlgFrame.mptr[index_vdec] , RINGBUF_VDEC_SIZE_2);    
    }
    else
    {
        m_mb = RK_MPI_MB_CreateBuffer(RINGBUF_VDEC_SIZE , RK_TRUE, 0); 
        ...
        MEDIA_BUFFER RGA_m_mb = RK_MPI_SYS_GetMediaBuffer(RK_ID_RGA, VDEC_CH, 500); //解码数据直接送到RGA
            if(!RGA_m_mb )
            {
                ...
                // {   
                //     if(H264_RESOL == RESOL_480P)
                //         fwrite(m_stVpssAlgFrame.mptr[index_vdec], 1, RINGBUF_VDEC_SIZE, fp);
                //     else if(H264_RESOL == RESOL_720P)
                //         fwrite(m_stVpssAlgFrame.mptr[index_vdec], 1, RINGBUF_VDEC_SIZE_2, fp);
                // }
                // fclose(fp);
