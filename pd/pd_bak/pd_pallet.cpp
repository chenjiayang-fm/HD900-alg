...
#define version "1126-00-V1.1-20240830"
...
void coordinate_remap(Algo_Result *stAlgResult_t,  Algo_Result_f stPdResult_t,uint8 Displaymode_t,uint32_t panel_width, uint32_t panel_height)
{
   ...
   switch (Displaymode_t)
   {
    ...
    else
    {
        ...
        if(stAlgResult_t->x1 > (panel_width-51) && stAlgResult_t->y2 < 90 )
        {
            stAlgResult_t->x1 = panel_width-50;
            stAlgResult_t->y2 = 90;
        }   
    ...
    {
        if(stAlgResult_t->x1 < (110+panel_width/2) && stAlgResult_t->y2 <58)
        {
            stAlgResult_t->x1 = (111+panel_width/2);
            stAlgResult_t->y2 = 59;
        }

        if(stAlgResult_t->x1 > (panel_width-52) && stAlgResult_t->y2 <58)
        {
            stAlgResult_t->x1 = (panel_width-53);
            stAlgResult_t->y2 = 59;
        }
    ...
    {
        if(stAlgResult_t->x1 < (110+panel_width/2) && stAlgResult_t->y2 <90)
        {
            stAlgResult_t->x1 = (111+panel_width/2);
            stAlgResult_t->y2 = 90;
        }
    ...
    else
    {
        ...
        if( stAlgResult_t->y1 <= 302)
        {
            stAlgResult_t->y1 = 302;
        }
    ...
    case DISPLAY_QUAD4:
        ...
        if(stAlgResult_t->x1 <  (110+panel_width/2) && stAlgResult_t->y1 < (58+panel_height/2))
        {
            stAlgResult_t->x1 = (111+panel_width/2);
            stAlgResult_t->y1 = (59+panel_height/2);
        }
...
void pd_Alarm_Out(void *pvArg)
{
    ...
    // printf("[%d],mode:%d",ALARM_OUTPUT_EN[SOURCE_ID],lmode);

    pd_Alarm_Out_Enable(lmode);

    // if( (lmode != PD_ALARM_TYPE_NULL && ALARM_OUTPUT_EN[0]) || (lmode != PD_ALARM_TYPE_NULL && ALARM_OUTPUT_EN[1]) || (lmode != PD_ALARM_TYPE_NULL && ALARM_OUTPUT_EN[2])|| (lmode != PD_ALARM_TYPE_NULL && ALARM_OUTPUT_EN[3]))
    // {
    //     if(lock == 0)
    //     {
    //         lmode = PD_ALARM_TYPE_RED;
...
// #endif
    ...
    {
       printf("type[%d]:%d,x1:%f,x2:%f,y1:%f,y2:%f,fConfidence:%f,uid:%d\r\n" ,i,stPdResult.stResults[i].classes, stPdResult.stResults[i].fX1,stPdResult.stResults[i].fX2,stPdResult.stResults[i].fY1,stPdResult.stResults[i].fY2,stPdResult.stResults[i].fConfidence,stPdResult.stResults[i].u32ID);
    }
    ...
    // for (i = 0; i < stPdResult.u32Nums; i++)
    // {
    //    printf("type[%d]:%d,x1:%f,x1:%f,y1:%f,y2:%f,fConfidence:%f,uid:%d\r\n" ,i,stPdResult.stResults[i].classes, stPdResult.stResults[i].fX1,stPdResult.stResults[i].fX2,stPdResult.stResults[i].fY1,stPdResult.stResults[i].fY2,stPdResult.stResults[i].fConfidence,stPdResult.stResults[i].u32ID);
    // }
...
skip_alg:  
        ...
        { 
            ...
            else if(stPdResult.stResults[i].classes == pdsa32::E_CLS_CAR || stPdResult.stResults[i].classes == pdsa32::E_CLS_PALLET)
            {
...
sh: can't create /sys/class/gpio_stonkam/RGB/rgbstate: nonexistent directory***********/
         ...
         //s32AlarmMode = PD_ALARM_TYPE_NULL;
            ...
            if((stPdResult.stResults[i].classes == pdsa32::E_CLS_CAR || stPdResult.stResults[i].classes == pdsa32::E_CLS_PALLET)&& stAlgResult[u8LenAlgData].alarm_type != PD_ALARM_TYPE_NULL )
                    u8car_cnt++;
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
   ...
   // Area_Info_init();
    ...
    s32Ret = pthread_create(&u32Tid_v4l2Save, &attr, mpp_vpss_FrameSave,NULL);//帧处理
...
toPcModel:
                if (NULL == apcsPdsAlgRgbPC)
                {
                    ...
                    apcsPdsAlgRgbPC = new pdsa32::CPdsAlg(pdsa32::E_PDSALG_TYPE_RGB_PC_PALLET,true);
                    if (NULL == apcsPdsAlgRgbPC)
                    {
                        print_level(SV_ERROR, "new pdsa32::CPdsAlg failed. [err=%d]\n", s32Ret);
                        return SV_FAILURE;
