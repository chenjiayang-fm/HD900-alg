...
float line_k1 = -(400.0/230);
float line_b1 = 180.0 + (400.0/230 *392);

float line_k2 = (400.0/230);
float line_b2 = 180.0 - (400.0/230 *632);

sint32 area_detect(Algo_Result result)
{
        int xr=0,xl=0;
        //1.用y2判断在哪个报警区域.  先用 y2 算出 xl ， xr 在矩形边上的左右边缘坐标
         if(result.y2 <= RED_BOTTOM_y && result.y2 >= GREEN_TOP_y)
         {
            xl = (int)(((float)result.y2 - line_b1) /line_k1);

            xr = (int)(((float)result.y2 - line_b2) /line_k2);

            //2. //在矩形框外， 则 return PD_ALARM_TYPE_NULL;
            if (result.x2 < xl || result.x1 > xr )
                {
                return PD_ALARM_TYPE_NULL;
                }
            else
                { 
                    if (result.y2 <= GREEN_BOTTOM_y)   //y2判断在哪个报警区域
                    {
                      return PD_ALARM_TYPE_GREEN;      
                    }
                    else if(result.y2 <= YELLOW_BOTTOM_y)
                    {
         ...
         else if (result.y2 > RED_BOTTOM_y && (result.x2 > RED_BOTTOM_x1  && result.x1 < RED_BOTTOM_x2) ) //3.  //防止y2 在梯形的底部时不报警
         {
           return  PD_ALARM_TYPE_RED;
         }
         else  //在梯形外不报警
...
sint32 mpp_vpss_GetAlgFrame()
{
    ...
    // PD_GUI_IMG_S  stLaneImgDst;
    // stLaneImgDst.s32Width =  PD_IMAGE_WIDTH;//480;
    // stLaneImgDst.s32Height = PD_IMAGE_HEIGHT;//288;
    // stLaneImgDst.pbmp = malloc(stLaneImgDst.s32Width * stLaneImgDst.s32Height * 3);
    // u32DstSize = stLaneImgDst.s32Width * stLaneImgDst.s32Height * 3;
    ...
    s32Ret = RK_MPI_VDEC_CreateChn(0, &stVdecAttr);
    if(s32Ret != RK_SUCCESS)
  {
      print_level(SV_ERROR, "RK_MPI_VDEC_CreateChn failed. [err=%#x]\n", s32Ret);

  }

    printf("*****************RK_MPI_VDEC_CreateChn success********************\n");
    ...
    while(1)   
    {
       ...
       s32BufIndex = readbuff.index;
#if 1

  //  printf("******RK_MPI_MB_CreateBuffer*******\n");
   MEDIA_BUFFER m_mb = RK_MPI_MB_CreateBuffer(readbuff.length, RK_TRUE, 0);  //yuv 420
    if(!m_mb)
    {
      print_level(SV_ERROR, "RK_MPI_MB_CreateBuffer failed\n");
    }

    memcpy(RK_MPI_MB_GetPtr(m_mb), m_stVpssAlgFrame.mptr[s32BufIndex] , readbuff.length);
 //   printf("******RK_MPI_MB_GetPtr*******\n");
    RK_MPI_MB_SetSize(m_mb, readbuff.length);
  //  printf("#Send packet(%p, %zuBytes) to VDEC[0].\n", RK_MPI_MB_GetPtr(m_mb),RK_MPI_MB_GetSize(m_mb));
     
 //    printf("******RK_MPI_SYS_SendMediaBuffer*******\n");
    s32Ret =  RK_MPI_SYS_SendMediaBuffer(RK_ID_VDEC, 0, m_mb);
    if(s32Ret != RK_SUCCESS)
    {
  ...
  //  printf("******RK_MPI_SYS_GetMediaBuffer*******\n");
    ...
    MEDIA_BUFFER RGA_m_mb = RK_MPI_SYS_GetMediaBuffer(RK_ID_RGA, 0, 50); //解码数据直接送到RGA
    if(!RGA_m_mb)
    {
        print_level(SV_ERROR, "RK_MPI_MB_ReleaseBuffer failed.\n");
        Reset_frame[*]        //重新收I帧
    }
 ...
 //    printf("******RK_MPI_MB_ReleaseBuffer*******\n");
        ...
        dstData[index][0]  = *(m_stVpssAlgFrame.mptr[s32BufIndex] + 32);
...
skip_alg:  
        ...
        */
        ...
        //如果把梯形检测区域关掉了，则返回全部
            s32AlarmMode = area_detect(stAlgResult[i]); //获取报警程度
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
   ...
   s32Ret = mpp_vpss_InitAlgChn();   //打开264的句柄  m_stVpssAlgFrame.s32AlgChnFd
    ...
    s32Ret = pthread_create(&u32Tid_v4l2, &attr, mpp_vpss_GetAlgFrame,NULL); //264解码
