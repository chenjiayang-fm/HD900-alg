...
sint32 mpp_vpss_Monitor()
{
    ...
    //断电上电 test
    #ifdef AUTO_TEST
        if(alg_count > 0 )  //从1开始(进算法开始计时)
        {
            alg_count++;
            if( (alg_count % 300) == 0)
            {
                printf(" alg run success \n ");
                            //将成功的结果写入log中
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
   ...
   uint32 u32Tid_vdec = 0; 
                    ...
                    FILE *fp = fopen("/root/test.log", "r+");
                    FILE *fp2 = fopen("/root/cnt", "r+"); //记录总的测试次数

                    uint32_t num ;

                     fscanf(fp2, "%d", &num); 
                     num ++;
                     fwrite(&num, 1, sizeof(uint32_t), fp2);
                     printf("wait alg run test :%d\n ",num);
