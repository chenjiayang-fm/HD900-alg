...
#define version "1126-00-V1.2-20250109"
...
uint8 DisplayMode = 0;
uint8 DETECT_MODE = 0;
uint8 ALARM_OUTPUT_EN[4] = {0};
uint8 BsdRange_TYPE = 0;
...
opticalflowanalyzeralg::STAlgParams stPalletDetectParams;

// [1] 算法初始化
// 模型路径
char pOFModelPath[50];
opticalflowanalyzeralg::STAlgModelPath stOFModelsPath(pOFModelPath);

char pPDModelPath[50];
opticalflowanalyzeralg::STAlgModelPath stPDModelsPath(pPDModelPath);
...
#define UVC_CAM_WIDTH   576
#define UVC_CAM_HEIGHT  320
...
typedef struct Detect_Area_Info {
    short line_top_x1;   
    short line_top_x2;
    short line_bottom_x1;
    short line_bottom_x2;
    short line_top_y;
    short line_second_y;
    short line_third_y;
    short line_bottom_y;
    Line_Info line_info;    //斜率 和 截距
    node p[4];
    unsigned char recive_flag; // 如果这个为0表示没有收到检测区域
} Area_Info;   //梯形检测区域


typedef struct Detect_Pallet_Info {
    short line_y;
...
void sendcmd2sonix(int port,int CH,int CMD) //CMD_RESET_IFRAME = 0,      CMD_AREA_INFO=1 , CMD_PALLET_INFO=2
{
   ...
   buf[2] = CH;
   buf[3] = CMD;
   write(m_stPdInfo.s32SerialFd[port], buf, 4);
   printf("sendcmd2sonix:%d\n", CMD);
}

void sendversion2sonix(int port)
{
   unsigned char buf[50] = {0};
   buf[0] = 0xff;  //包头
   buf[1] = 0xdd;  //包头
   sprintf(&buf[2],"%s",version);
   
   write(m_stPdInfo.s32SerialFd[port], buf, sizeof(buf));

   printf("sendcmd2sonix:%s\n",buf);
}

int search_CH(char *src, char *sub)
{
    int i,j=0,flag=-1,k;
    while(j!=strlen(src))
    {
...
Area_Info Area_Info_Get[4] = {0};
Area_Info Area_Info_Get_init[4] = {0};
Pallet_Info Pallet_Info_Get = {0};
...
{
    AreaInfo->line_info.line_k1 = get_line_k(AreaInfo->line_bottom_x1, AreaInfo->line_bottom_y, AreaInfo->line_top_x1, AreaInfo->line_top_y);
    AreaInfo->line_info.line_b1 = get_line_b(AreaInfo->line_bottom_x1, AreaInfo->line_bottom_y, AreaInfo->line_top_x1, AreaInfo->line_top_y);
    AreaInfo->line_info.line_k2 = get_line_k(AreaInfo->line_bottom_x2, AreaInfo->line_bottom_y, AreaInfo->line_top_x2, AreaInfo->line_top_y);
    AreaInfo->line_info.line_b2 = get_line_b(AreaInfo->line_bottom_x2, AreaInfo->line_bottom_y, AreaInfo->line_top_x2, AreaInfo->line_top_y);
}
...
void Area_Info_init(){
    ...
    for(uint i = 0; i < 4 ; i++)
    {
       Area_Info_Get_init[i].line_top_x1    =  390;
       Area_Info_Get_init[i].line_top_x2    =  640;
       Area_Info_Get_init[i].line_bottom_x1 =  165;
       Area_Info_Get_init[i].line_bottom_x2 =  865;
       Area_Info_Get_init[i].line_top_y     =  180;
       Area_Info_Get_init[i].line_second_y  =  315;
       Area_Info_Get_init[i].line_third_y   =  450; 
       Area_Info_Get_init[i].line_bottom_y  =  580;

        get_line_info(&Area_Info_Get_init[i]);

        Area_Info_Get_init[i].recive_flag = 1;
...
sint32 area_detect(Algo_Result result , Area_Info AreaInfo)
{
        ...
        static unsigned char cnt = 100;

    if(DETECT_AREA_EN)
    {  
        if(AreaInfo.recive_flag)  
        {
        //1.用y2判断在哪个报警区域.  先用 y2 算出 xl ， xr 在矩形边上的左右边缘坐标
        
         if(result.y2 <= AreaInfo.line_bottom_y && result.y2 >= AreaInfo.line_top_y)
         {
            if(AreaInfo.line_info.line_k1 != 0)
                xl_top = (int)(((float)result.y2 - AreaInfo.line_info.line_b1) / AreaInfo.line_info.line_k1);
            else
                xl_top = AreaInfo.line_top_x1;

            if(AreaInfo.line_info.line_k2 != 0)
                xr_top = (int)(((float)result.y2 - AreaInfo.line_info.line_b2) / AreaInfo.line_info.line_k2);
            else
                xr_top = AreaInfo.line_top_x2;
         ...
         {
            if(AreaInfo.line_info.line_k1 != 0)
                xl_bottom = (int)(((float)result.y1 - AreaInfo.line_info.line_b1) / AreaInfo.line_info.line_k1);
            else
                xl_bottom = AreaInfo.line_bottom_x1;

            if(AreaInfo.line_info.line_k2 != 0)
                xr_bottom = (int)(((float)result.y1 - AreaInfo.line_info.line_b2) / AreaInfo.line_info.line_k2);
            else
                xr_bottom = AreaInfo.line_bottom_x2;

                        //2. //在矩形框外， 则 return PD_ALARM_TYPE_NULL;
            if (result.x1 < xl_bottom || result.x1 > xr_bottom )
                {
                return PD_ALARM_TYPE_NULL;
                }
            else
         ...
         else if(result.y1 < AreaInfo.line_top_y && result.y2 > AreaInfo.line_bottom_y )   //y1 < 梯形上部，y2 > 梯形下部
         {
            if((result.x2 > AreaInfo.line_bottom_x1 && result.x2 > AreaInfo.line_bottom_x2) || (result.x1 < AreaInfo.line_bottom_x2 && result.x1 > AreaInfo.line_bottom_x1) )
            {
               if(result.y2 > AreaInfo.line_third_y)
                return PD_ALARM_TYPE_RED; 
               else if (result.y2 > AreaInfo.line_second_y)
                return PD_ALARM_TYPE_YELLOW; 
               else
                return PD_ALARM_TYPE_GREEN; 
            }
    ...
    {
        //if(result.y2 <= AreaInfo.line_second_y)
        if(result.y2 <= 600 / 3)
             return PD_ALARM_TYPE_GREEN; 
        //else if(result.y2 <= AreaInfo.line_third_y)
        else if(result.y2 <=600/3*2)
            return PD_ALARM_TYPE_YELLOW; 
        else
            return PD_ALARM_TYPE_RED; 
...
short getCross(node p1,node p2,node p){ 

    //return ( ((p2.x-p1.x)*(p.y-p1.y)-(p.x-p1.x)*(p2.y-p1.y)) >=0? 1:-1);
...
sint32 area_detect(Algo_Result result , Area_Info AreaInfo)
{
    ...
    if(DETECT_AREA_EN)
    {
      if( (getCross(AreaInfo.p[0],AreaInfo.p[1],p_l)*getCross(AreaInfo.p[2],AreaInfo.p[3],p_l)>=0 && getCross(AreaInfo.p[1],AreaInfo.p[2],p_l)*getCross(AreaInfo.p[3],AreaInfo.p[0],p_l)>=0) \
        || (getCross(AreaInfo.p[0],AreaInfo.p[1],p_r)*getCross(AreaInfo.p[2],AreaInfo.p[3],p_r)>=0 && getCross(AreaInfo.p[1],AreaInfo.p[2],p_r)*getCross(AreaInfo.p[3],AreaInfo.p[0],p_r)>=0) )
       {
            if(result.y2 <= AreaInfo.line_second_y)
                return PD_ALARM_TYPE_GREEN; 
            else if(result.y2 <= AreaInfo.line_third_y)
...
void coordinate_remap(Algo_Result *stAlgResult_t,  Algo_Result_f stPdResult_t,uint8 Displaymode_t,uint32_t panel_width, uint32_t panel_height)
{
   ...
   switch (Displaymode_t)
   {
    ...
    if(BsdRange_TYPE == 0)
    {
        if(stAlgResult_t->x1 < 110 && stAlgResult_t->y1 <80 )
        {
            stAlgResult_t->x1 = 111;
            stAlgResult_t->y1 = 81;
        }
        
        if(stAlgResult_t->x1 > (panel_width-51) && stAlgResult_t->y1 < 80 )
        {
            stAlgResult_t->x1 = panel_width-50;
            stAlgResult_t->y1 = 81;
        }
    }
    else
    {
        if(stAlgResult_t->x1 < 110 && stAlgResult_t->y1 <90 )
        {
            stAlgResult_t->x1 = 111;
            stAlgResult_t->y1 = 90;
        }
        
        if(stAlgResult_t->x2 > (panel_width-50) && stAlgResult_t->y1 < 90 )
        {
            stAlgResult_t->x2 = panel_width-49;
            stAlgResult_t->y1 = 90;
        }   
    }
    break;
    ...
    if(BsdRange_TYPE == 0)
    {
        ...
        if(stAlgResult_t->x2 >= 510)
        {
            stAlgResult_t->x2 = 510;
        }
    }
    else
    {
        if(stAlgResult_t->x1 <= 110 && stAlgResult_t->y2 < 90)
        {
            stAlgResult_t->x2 = stAlgResult_t->x2 + (111- stAlgResult_t->x1);
            stAlgResult_t->x1 = 111;
            stAlgResult_t->y2 = 90;
        }
        ...
        if(stAlgResult_t->x2 >= 510)
        {
            stAlgResult_t->x1 = stAlgResult_t->x1 - (stAlgResult_t->x2 - 510);
            stAlgResult_t->x2 = 510;
        }
    }
    ...
    if(BsdRange_TYPE == 0)
    {    
        if(stAlgResult_t->x1 < (110+panel_width/2) && stAlgResult_t->y1 <80)
        {
            stAlgResult_t->x1 = (111+panel_width/2);
            stAlgResult_t->y1 = 51;
        }

        if(stAlgResult_t->x1 > (panel_width-52) && stAlgResult_t->y1 <50)
        {
            stAlgResult_t->x1 = (panel_width-53);
            stAlgResult_t->y1 = 51;
        }
    }
    else
    {
        if(stAlgResult_t->x1 <= (110+panel_width/2) && stAlgResult_t->y2 < 89)
        {
            stAlgResult_t->x2 = stAlgResult_t->x2 + ((111+panel_width/2) - stAlgResult_t->x1);
            stAlgResult_t->x1 = (111+panel_width/2);

            stAlgResult_t->y2 = 90;
        }

        if(stAlgResult_t->x2 > (panel_width-52) && stAlgResult_t->y2 <58)
        {
            stAlgResult_t->x1 = stAlgResult_t->x1 - (stAlgResult_t->x2 - (panel_width-50));
            stAlgResult_t->x2 = (panel_width-50);
            stAlgResult_t->y2 = 59;
        }

        if(stAlgResult_t->x1 <= 514)
        {
            stAlgResult_t->x2 = stAlgResult_t->x2 - (514-stAlgResult_t->x1);
            stAlgResult_t->x1 = 514;
        }
    }
    break;
    ...
    if(BsdRange_TYPE == 0)
    {
        ...
        if(stAlgResult_t->x1 <= 120 && stAlgResult_t->y1 <80)
        {
            stAlgResult_t->x1 = 120;
    ...
    else
    {
        ...
        if( stAlgResult_t->y2 >= 298)
        {
            stAlgResult_t->y2 = 298;
        }
    ...
    if(BsdRange_TYPE == 0)
    {
        ...
        if(stAlgResult_t->x1 < 110 && stAlgResult_t->y1 < (50+panel_height/2))
        {
            stAlgResult_t->x1 = 111;
            stAlgResult_t->y1 = (51+panel_height/2);
        }
    }
    else
    {
        if(stAlgResult_t->x1 <= 110 && stAlgResult_t->y2 < (58+panel_height/2))
        {
            stAlgResult_t->x2 = stAlgResult_t->x2 + (111 - stAlgResult_t->x1);
            stAlgResult_t->x1 = 111;

            stAlgResult_t->y2 = (59+panel_height/2);
        }
        ...
        if( stAlgResult_t->y2 <= 302)
        {
            stAlgResult_t->y2 = 302;
        }
    ...
    case DISPLAY_QUAD4:
        ...
        if(stAlgResult_t->x1 <=  (110+panel_width/2) && stAlgResult_t->y2 < (58+panel_height/2))
        {
            stAlgResult_t->x2 = stAlgResult_t->x2 + ((111+panel_width/2) - stAlgResult_t->x1);
            stAlgResult_t->x1 = (111+panel_width/2);
            stAlgResult_t->y2 = (59+panel_height/2);
...
TRWRingQueue<int> ringque_V4L2(UVC_BUF_NUM-1);

TRWRingQueue<int> ringque_MIDDLE(RINGBUF_VDEC_COUNT-1); //V4L2 中间层
...
sint32 mpp_vpss_InitAlgChn()
{
    ...
    // char cmdStr[128] = {0};
    // uint16 cnt = 0;
    // if(access("/root/AlgCntFile", F_OK) == 0) {  
    //      FILE * file00 = fopen("/root/AlgCntFile", "r+"); 
    //      fread(&cnt, sizeof(uint16),1,file00);
    //      printf("***************\r\n");
    //      printf("cnt ALG:%d\r\n",cnt);
    //      printf("***************\r\n");
    //      cnt++;
    //      fseek(file00,0,SEEK_SET);
    //      fwrite(&cnt,sizeof(uint16),1,file00);
    //      fclose(file00); 
         
    // } else {
    //      sprintf(cmdStr, "touch /root/AlgCntFile");
    //      SAFE_SV_System(cmdStr);
    // }
...
{   
    ...
    memcpy(v4l2_node,path,sizeof(v4l2_node));
    // //打开设备
...
unsigned char MiddleData[RINGBUF_VDEC_COUNT][RINGBUF_VDEC_SIZE_2];
int s32time_out = 0;
...
sint32 mpp_vpss_GetAlgFrame()
{
 ...
 //   sint32 frame_skip = 10; //前10帧不做检测
    ...
    sint32 frame_skip = 200; //前20帧不做检测
...
//  uint8_t skip_frame = 10; //初始的帧会不连续，需要跳过前几帧
    ...
    //     }
     ...
     //struct timeval timestamp = readbuff.timestamp;
    //  printf("[%d]Frame timestamp: %ld.%06ld\n",s32BufIndex, timestamp.tv_sec, timestamp.tv_usec); //时间戳
    ...
    int rel = ringque_V4L2.Write(s32BufIndex, [](int &release){     
    return ++release; });//直接发s32BufIndex
...
sint32 mpp_vpss_FrameSave()
{
    ...
    while(1)
    {
        ...
        index_v4l2 = ringque_V4L2.Read(chn, timeout, [/*UtilizeResource*/&ringque_VDEC](int &current){
            return current; //0;
        });

    if(H264_RESOL == RESOL_480P)
    {
        memcpy(MiddleData[index],m_stVpssAlgFrame.mptr[index_v4l2],RINGBUF_VDEC_SIZE);
    }
    else if(H264_RESOL == RESOL_720P)
    {
        memcpy(MiddleData[index],m_stVpssAlgFrame.mptr[index_v4l2], RINGBUF_VDEC_SIZE_2);    
    }

   // printf("vdec now %d,last %d\n",MiddleData[index][7],MiddleData[Index_temp][7]);

    //打印送帧时间
    // Frm_count = MiddleData[index][7];
    // printf("[%d]frame%d,%dms \n",index, MiddleData[index][7],u32StepTimeMs);

    uint8_t Getframe_CH = *(MiddleData[index] +32);  

    //切换分割 模式
    #if 1
        if(! (*(MiddleData[index] +34)&0x40) ) //切到单通道变成720p
        {
...
//通知重发I帧
    #if 1
        if(Reset_Frame_done[Getframe_CH] < 2)
        {
            if( *(MiddleData[index] +6) == 0)
            {   
                Reset_Frame_done[Getframe_CH] ++;
                printf("Reset_Frame_done:%d \n",Getframe_CH);

                    if(Reset_Frame_done[Getframe_CH] <2) //收到第二个I帧才退出
                        continue;
            }
            else
            {
                ...
                sendcmd2sonix(0,Getframe_CH,CMD_RESET_IFRAME);
...
#if 1
    // if(MiddleData[index][7] == 27) //改帧数据测试
    // {
    //     for(uint8_t i = 0; i<9 ;i++)
    //     MiddleData[index][RINGBUF_VDEC_SIZE-10 +i] = 0xaa;
    // }

     int rel = ringque_MIDDLE.Write(index, [](int &release){     
 return ++release; });//直接发 s32BufIndex 
    ...
    else
    {
        if(frame_sequence > 1)
        {
            if( !(frame_sequence_detect(MiddleData[index][7],MiddleData[temp1][7])) )
            {
                print_level(SV_ERROR, "frame sequence still err1 :%d ,%d,%d \n",MiddleData[index][7],MiddleData[temp1][7],MiddleData[temp2][7] ); //说明此时连续跳了两帧 25 27 28 26
                sleep_ms(100);
                system("pkill -9  alg");
            }
            else
            {
            print_level(SV_ERROR, "frame 3 sequence:%d,%d,%d \n",MiddleData[index][7],MiddleData[temp1][7],MiddleData[temp2][7]);

            int rel1 = ringque_MIDDLE.Write(index, [](int &release){      //送第一帧  
            return ++release; });//直接发 s32BufIndex  

            int rel2 = ringque_MIDDLE.Write(temp1, [](int &release){      //送第二帧  
            return ++release; });//直接发 s32BufIndex  

            int rel3 = ringque_MIDDLE.Write(temp2, [](int &release){      //送第三帧  
            return ++release; });//直接发 s32BufIndex  
        ...
        else if(frame_sequence)
        {
            if( !(frame_sequence_detect(MiddleData[index][7],MiddleData[Index_temp][7]))) //帧序号倒过来后仍然不对
            {
                print_level(SV_ERROR, "frame sequence still err0 :%d ,%d \n",MiddleData[index][7],MiddleData[Index_temp][7] ); //说明此时连续跳了两帧 25 27 28 26
 
                #if 0    //在这里重发I帧有概率还是解码失败
                    for (uint8_t i = 0; i < 4; i++)
                    {
                       Reset_Frame_done[i] = 0;
                    }
                    ...
                    frame_skip = 200;
                #else
                        if(counter % RINGBUF_VDEC_COUNT != index)
                         counter ++;
                        else
                         counter = (index + 1) % RINGBUF_VDEC_COUNT;  
            ...
            {
                print_level(SV_ERROR, "frame sequence repair succ\n");
            int rel1 = ringque_MIDDLE.Write(index, [](int &release){     
                    return ++release; }); //先发当前帧 index
            int rel2 = ringque_MIDDLE.Write(Index_temp, [](int &release){     
                    return ++release; }); //再发上一帧 Index_temp
        ...
        else if( !(frame_sequence_detect(MiddleData[Index_temp][7],MiddleData[index][7])))
           {
            print_level(SV_ERROR, "frame sequence err now:%d,last:%d\n", MiddleData[index][7] ,MiddleData[Index_temp][7]); //帧倒序
            frame_sequence = 1;       
...
sint32 mpp_vdec_init()
{
    ...
    for(int i = 0; i<4 ; i++)
    {
        ...
        if(s32Ret != RK_SUCCESS)
        {
            ...
            //640x480  ->  640x370 -> 608x352 
            RGA_ATTR_S stRgaAttr;
            stRgaAttr.bEnBufPool = RK_TRUE;
            stRgaAttr.u16BufPoolCnt = 3;
            stRgaAttr.u16Rotaion = 0;
            stRgaAttr.stImgIn.u32X = 0;
            stRgaAttr.stImgIn.u32Y = 0;  //上面裁剪30个像素
            stRgaAttr.stImgIn.imgType = IMAGE_TYPE_NV12;
            stRgaAttr.stImgIn.u32Width = u32Width;     //要裁剪的宽高
            stRgaAttr.stImgIn.u32Height = u32Height;  //下面裁剪80个像素
            stRgaAttr.stImgIn.u32HorStride = u32Width;  //原始输入分辨率
...
sint32 mpp_vpss_Vdec()
{
   ...
   // sint32 s32Fd = m_stVpssAlgFrame.s32AlgChnFd;
    ...
    uint8 Getframe_CH = 0;
    uint8 Detect_en_flag = 0;    
    uint8 DisplayMode_flag = 0;
    uint8 DetectMode_flag = 0;
    uint8 Alarm_en_flag =0;
    uint8 VDEC_CH = 0; 
    uint8 BsdRange_type_flag = 0;
    ...
    while(1)   
    {
        ...
        index_vdec = ringque_MIDDLE.Read(chn, timeout, [/*UtilizeResource*/&ringque_VDEC](int &current){
            return current; //0;
        });

        //printf("vdec read[%d] :%d \n",index_vdec, *(m_stVpssAlgFrame.mptr[index_vdec] +7));
    ...
    Getframe_CH = *(MiddleData[index_vdec] + 32); 
    Detect_en_flag = *(MiddleData[index_vdec] + 38);
    DisplayMode_flag = *(MiddleData[index_vdec] + 8);
    DetectMode_flag = *(MiddleData[index_vdec] + 9);
    Alarm_en_flag = *(MiddleData[index_vdec] + 10);
    BsdRange_type_flag = *(MiddleData[index_vdec] + 11);
    ...
    if(H264_RESOL)//== RESOL_720P
        ...
        //printf("vdec read[%d][%d] \n",Getframe_CH,MiddleData[index_vdec][7]);
    ...
    if(H264_RESOL == RESOL_480P)
    {
        m_mb = RK_MPI_MB_CreateBuffer(RINGBUF_VDEC_SIZE , RK_TRUE, 0);  
        if(!m_mb)
        {
          print_level(SV_ERROR, "RK_MPI_MB_CreateBuffer failed\n");
        }
        RK_MPI_MB_SetSize(m_mb, RINGBUF_VDEC_SIZE );
        memcpy(RK_MPI_MB_GetPtr(m_mb), (MiddleData[index_vdec]+48), (RINGBUF_VDEC_SIZE-48) );
    }
    else if(H264_RESOL == RESOL_720P)
    {
        m_mb = RK_MPI_MB_CreateBuffer(RINGBUF_VDEC_SIZE_2 , RK_TRUE, 0); 
        if(!m_mb)
        {
          print_level(SV_ERROR, "RK_MPI_MB_CreateBuffer failed\n");
        }
        RK_MPI_MB_SetSize(m_mb, RINGBUF_VDEC_SIZE_2);
        memcpy(RK_MPI_MB_GetPtr(m_mb), (MiddleData[index_vdec]+48) , (RINGBUF_VDEC_SIZE_2-48) );    
    }
    else
    {
        m_mb = RK_MPI_MB_CreateBuffer(RINGBUF_VDEC_SIZE , RK_TRUE, 0); 
        ...
        // printf("[%d]vdec%d,%dms\n",index_vdec,MiddleData[index_vdec][7],u32StepTimeMs);

        MEDIA_BUFFER RGA_m_mb = RK_MPI_SYS_GetMediaBuffer(RK_ID_RGA, VDEC_CH, 10000); //解码数据直接送到RGA
            if(!RGA_m_mb )
            {
                ...
                {   
                    if(H264_RESOL == RESOL_480P)
                        fwrite(MiddleData[index_vdec], 1, RINGBUF_VDEC_SIZE, fp);
                    else if(H264_RESOL == RESOL_720P)
                        fwrite(MiddleData[index_vdec], 1, RINGBUF_VDEC_SIZE_2, fp);
                }
                fclose(fp);
            ...
            dstData[index][RINGBUF_SIZE-extern_num+3] =  DetectMode_flag;
            dstData[index][RINGBUF_SIZE-extern_num+4] =  Alarm_en_flag;
            dstData[index][RINGBUF_SIZE-extern_num+5] =  BsdRange_type_flag;
...
sint32 mpp_vpss_Monitor()
{
    ...
    char buf[128] = {0};
    sint32 ret;
    sprintf(cmdStr, "ls %s", v4l2_node);

    while(1)
    {
    s32time_out ++;
    if(s32time_out > 100) //60s没有进入算法，则重启
    {
    ...
    //****************监控video节点*******************
        ...
        ret =  SAFE_System_Recv(cmdStr, buf, sizeof(buf));
        if(ret < 0)
        {
         print_level(SV_ERROR, "find video_node err\n");
         mpp_vpss_Deinitialize();
         system("pkill -9  alg"); 
        }
        else
        {
            if( search_CH(buf,v4l2_node) == -1)
            {
                 print_level(SV_ERROR, "find video_node err\n");
                 mpp_vpss_Deinitialize();
                 system("pkill -9  alg"); 
    ...
    // if(Area_Info_Get[CH].recive_flag == 0)  
    //     sendcmd2sonix(0,CH,CMD_AREA_INFO);  //请求发送区域信息
    // CH ++;
    // CH = CH % 4;
    if(Pallet_Info_Get.recive_flag == 0)
        sendcmd2sonix(0, 0, CMD_PALLET_INFO);  //请求发送栈板参考线信息，CAM1
...
static void AlarmRangSetup(uint8_t tRateRange)
{
    printf("recieve new pallet tRateRange:%d\n", tRateRange);
    stPalletDetectParams.stAlarmRang.u32Range1 = 10 * tRateRange;
    stPalletDetectParams.stAlarmRang.u32Range2 = 20 * tRateRange;
    stPalletDetectParams.stAlarmRang.u32Range3 = 30 * tRateRange;
}

opticalflowanalyzeralg::COpticalFlowAnalyzer cPalletAnalyzer(opticalflowanalyzeralg::E_ALG_TYPE_PALLET_DETECT);
int uart_receive()
{
    sint32 s32Ret = 0;
    fd_set rfds;
    ...
    int buflen = 0;
    int bufindex = 0;
    unsigned char buf_temp[255] = {0};
    unsigned char buf[255] = {0};
    unsigned char ch = 0;
    unsigned char flag =0; //还没收到数据/刚收完一帧数据
    uint16_t u16Crc = 0;
    uint8_t size = 0;   
    ...
    if(s32Ret > 0)
       {
        if(FD_ISSET(m_stPdInfo.s32SerialFd[0],&rfds))
            {
            rxlen = read(m_stPdInfo.s32SerialFd[0] , buf_temp , 255);
            if(rxlen > 0) 
            {     
                if(flag == 0)
                {

                    if(buf_temp[0] == 0xff && buf_temp[1] == 0xdd) //发版本号
                    {
                        sendversion2sonix(0);
                    }

                    if(buf_temp[0] == 0xff && buf_temp[1] == 0xbb) //检测区域信息
                    {
                        bufindex = 0;
                        if(rxlen >= (buf_temp[2]) )//一次性全收完了                
                        {
                            printf("buf:");    
                            ...
                            u16Crc = crc16_Gen(&buf_temp[2], buf_temp[2]-4);//减去包头和crc
                            printf("crc: %x - %x\n", u16Crc,((buf_temp[(buf_temp[2]-1)]<< 8) + buf_temp[buf_temp[2]-2]));
                            if(u16Crc == ((buf_temp[(buf_temp[2]-1)]<< 8) + buf_temp[buf_temp[2]-2]) )
                            {  //crc校验通过
                                ch = buf_temp[3];    
                            #if 0
                                size = buf[8];
                                Area_Info_Get[ch].line_top_x1     = buf_temp[5]  +  (buf_temp[4] <<8) + 113;
                                Area_Info_Get[ch].line_top_y      = buf_temp[7]  +   (buf_temp[6]<<8);
                                Area_Info_Get[ch].line_top_x2     = Area_Info_Get[ch].line_top_x1 + 100 + 40*(size - 1);
                                Area_Info_Get[ch].line_bottom_y   = Area_Info_Get[ch].line_top_y + 294 ;
                                Area_Info_Get[ch].line_bottom_x1  = Area_Info_Get[ch].line_top_x1 - 113;
                                Area_Info_Get[ch].line_bottom_x2  = Area_Info_Get[ch].line_top_x2 + 113;
                                Area_Info_Get[ch].line_second_y   = Area_Info_Get[ch].line_top_y + (Area_Info_Get[ch].line_bottom_y - Area_Info_Get[ch].line_top_y) / 3 ;
                                Area_Info_Get[ch].line_third_y    = Area_Info_Get[ch].line_top_y + (Area_Info_Get[ch].line_bottom_y - Area_Info_Get[ch].line_top_y) / 3 *2 ;
                        
                                get_line_info(&Area_Info_Get[ch]);
                                //获得梯形的四个点的坐标
                                Area_Info_Get[ch].p[0].x  = Area_Info_Get[ch].line_top_x1;
                                Area_Info_Get[ch].p[0].y  = Area_Info_Get[ch].line_top_y;
                                Area_Info_Get[ch].p[1].x  = Area_Info_Get[ch].line_top_x2;
                                Area_Info_Get[ch].p[1].y  = Area_Info_Get[ch].line_top_y;
                                Area_Info_Get[ch].p[2].x  = Area_Info_Get[ch].line_bottom_x1;
                                Area_Info_Get[ch].p[2].y  = Area_Info_Get[ch].line_bottom_y;
                                Area_Info_Get[ch].p[3].x  = Area_Info_Get[ch].line_bottom_x2;
                                Area_Info_Get[ch].p[3].y  = Area_Info_Get[ch].line_bottom_y;
                            #else
                                Area_Info_Get[ch].line_top_x1      =  (buf_temp[5] <<8) +  (buf_temp[4]   );
                                Area_Info_Get[ch].line_top_x2      =  (buf_temp[7] <<8) +  (buf_temp[6]   );
                                Area_Info_Get[ch].line_bottom_x1   =  (buf_temp[9] <<8) +  (buf_temp[8]   );
                                Area_Info_Get[ch].line_bottom_x2   =  (buf_temp[11]<<8)  +  (buf_temp[10] );
                                Area_Info_Get[ch].line_top_y       =  (buf_temp[13]<<8)  +  (buf_temp[12] );
                                Area_Info_Get[ch].line_bottom_y    =  (buf_temp[19]<<8)  +  (buf_temp[18] );

                                Area_Info_Get[ch].line_second_y    = Area_Info_Get[ch].line_top_y + (Area_Info_Get[ch].line_bottom_y -Area_Info_Get[ch].line_top_y ) *((buf_temp[15]<<8)  +  (buf_temp[14] )) / 12 ;
                                Area_Info_Get[ch].line_third_y     = Area_Info_Get[ch].line_top_y + (Area_Info_Get[ch].line_bottom_y -Area_Info_Get[ch].line_top_y ) *((buf_temp[17]<<8)  +  (buf_temp[16] )) / 12 ;


                                get_line_info(&Area_Info_Get[ch]);
                            #endif
                                printf("recieve new area info0[%d],x1:%d,x2:%d,x3:%d,x4:%d,y1:%d,y2:%d,y3:%d,y4:%d\n",buf_temp[3], Area_Info_Get[buf_temp[3]].line_top_x1 ,Area_Info_Get[buf_temp[3]].line_top_x2 ,\
                                Area_Info_Get[buf_temp[3]].line_bottom_x1 , Area_Info_Get[buf_temp[3]].line_bottom_x2,Area_Info_Get[buf_temp[3]].line_top_y ,Area_Info_Get[buf_temp[3]].line_second_y ,\
                                Area_Info_Get[buf_temp[3]].line_third_y , Area_Info_Get[buf_temp[3]].line_bottom_y );
                                Area_Info_Get[buf_temp[3]].recive_flag = 1;   
                                //sendcmd2sonix(0,buf_temp[3],CMD_AREA_COMFIRM);
                            }
                        }
                        else //后面还有数据
                        {
                            memcpy(&buf[bufindex],buf_temp,rxlen);  
                            bufindex = bufindex + rxlen;
                            flag = 1; //开始收到数据
                        }
                    }

                    if(buf_temp[0] == 0xff && buf_temp[1] == 0xcc) //栈板检测参考线信息
                    {
                        bufindex = 0;
                        if(rxlen >= (buf_temp[2]))//一次性全收完了                
                        {
                            ...
                            {  //crc校验通过
                                ch = buf_temp[3];
                                printf("receive cam channel = %d\n", ch);
                                //只支持CAM1
                                if(ch == 0)
                                {
                                    Pallet_Info_Get.line_y = (buf_temp[5]<<8) +  (buf_temp[4]);
                                    printf("recieve new pallet info0[%d],y:%d\n",buf_temp[3], Pallet_Info_Get.line_y);

                                    Pallet_Info_Get.recive_flag = 1;
                                    
                                    //图像变换
                                    stPalletDetectParams.u32CaliLine = (uint32_t)(((float)Pallet_Info_Get.line_y / 600.0f) * 352.0f);
                                    printf("recieve new pallet TurnoffTime:%d\n",buf_temp[6]);
                                    stPalletDetectParams.u32TurnoffTime = buf_temp[6];
                                    printf("recieve new pallet FlowFrameInterval:%d\n",buf_temp[7]);
                                    stPalletDetectParams.u32FlowFrameInterval = buf_temp[7];

                                    AlarmRangSetup(buf_temp[8]);

                                    printf("recieve new pallet fFLowPauseDuration:%d\n",buf_temp[9]);
                                    stPalletDetectParams.fFLowPauseDuration = buf_temp[9];
                                    printf("recieve new pallet fFlowRunDuration:%d\n",buf_temp[10]);
                                    stPalletDetectParams.fFlowRunDuration = buf_temp[10];
                                    
                                    printf("recieve new pallet fPalletDetectSensity:%d\n",buf_temp[11]);
                                    stPalletDetectParams.fPalletDetectSensity = (float)buf_temp[11]/100.0f;

                                    cPalletAnalyzer.SaveParams(PALLET_CONFIG, stPalletDetectParams);
                                }
                            }
                        }
                        else //后面还有数据
                ...
                else
                {
                    ...
                    if( bufindex  >= (buf[2]))  // 第2位是包长度  //收完了完整的包
                    {
                        //收完整个包开始处理
                        printf("buf:");   
                        ...
                        u16Crc = crc16_Gen(&buf[2], buf[2]-4); //减去包头和crc
                        printf("crc: %x - %x\n", u16Crc,((buf[(buf[2]-1)]<< 8) + buf[buf[2]-2]));

                        if(u16Crc == ((buf[(buf[2]-1)]<< 8) + buf[buf[2]-2]))
                        {  //crc校验通过
                            ch = buf[3];
                            ...
                            if(buf_temp[0] == 0xff && buf_temp[1] == 0xbb) //检测区域信息
                            {
                                #if 0     
                                size = buf[8];    
                                Area_Info_Get[ch].line_top_x1     = buf[5]  +  (buf[4] <<8) + 113;
                                Area_Info_Get[ch].line_top_y      = buf[7]  +   (buf[6]<<8);
                                Area_Info_Get[ch].line_top_x2     = Area_Info_Get[ch].line_top_x1 + 100 + 40*(size - 1);
                                Area_Info_Get[ch].line_bottom_y   = Area_Info_Get[ch].line_top_y + 294 ;
                                Area_Info_Get[ch].line_bottom_x1  = Area_Info_Get[ch].line_top_x1 - 113;
                                Area_Info_Get[ch].line_bottom_x2  = Area_Info_Get[ch].line_top_x2 + 113;
                                ...
                                get_line_info(&Area_Info_Get[ch]);
                                //获得梯形的四个点的坐标
                                Area_Info_Get[ch].p[0].x  = Area_Info_Get[buf[3]].line_top_x1;
                                Area_Info_Get[ch].p[0].y  = Area_Info_Get[buf[3]].line_top_y;
                                Area_Info_Get[ch].p[1].x  = Area_Info_Get[buf[3]].line_top_x2;
                                Area_Info_Get[ch].p[1].y  = Area_Info_Get[buf[3]].line_top_y;
                                Area_Info_Get[ch].p[2].x  = Area_Info_Get[buf[3]].line_bottom_x1;
                                Area_Info_Get[ch].p[2].y  = Area_Info_Get[buf[3]].line_bottom_y;
                                Area_Info_Get[ch].p[3].x  = Area_Info_Get[buf[3]].line_bottom_x2;
                                Area_Info_Get[ch].p[3].y  = Area_Info_Get[buf[3]].line_bottom_y;
                            #else
                                Area_Info_Get[ch].line_top_x1      =  (buf[5] <<8) +  (buf[4]   );
                                Area_Info_Get[ch].line_top_x2      =  (buf[7] <<8) +  (buf[6]   );
                                Area_Info_Get[ch].line_bottom_x1   =  (buf[9] <<8) +  (buf[8]   );
                                Area_Info_Get[ch].line_bottom_x2   =  (buf[11]<<8)  +  (buf[10] );
                                Area_Info_Get[ch].line_top_y       =  (buf[13]<<8)  +  (buf[12] );
                                // Area_Info_Get[ch].line_second_y    =  (buf[15]<<8)  +  (buf[14] );
                                // Area_Info_Get[ch].line_third_y     =  (buf[17]<<8)  +  (buf[16] );
                                Area_Info_Get[ch].line_bottom_y    =  (buf[19]<<8)  +  (buf[18] );

                                Area_Info_Get[ch].line_second_y    = Area_Info_Get[ch].line_top_y + (Area_Info_Get[ch].line_bottom_y -Area_Info_Get[ch].line_top_y ) *((buf[15]<<8)  +  (buf[14] )) / 12 ;
                                Area_Info_Get[ch].line_third_y     = Area_Info_Get[ch].line_top_y + (Area_Info_Get[ch].line_bottom_y -Area_Info_Get[ch].line_top_y ) *((buf[17]<<8)  +  (buf[16] )) / 12 ;

                                get_line_info(&Area_Info_Get[ch]);
                            #endif
                                printf("recieve new area info1[%d],x1:%d,x2:%d,x3:%d,x4:%d,y1:%d,y2:%d,y3:%d,y4:%d\n",buf[3], Area_Info_Get[buf[3]].line_top_x1 ,Area_Info_Get[buf[3]].line_top_x2 ,\
                                Area_Info_Get[buf[3]].line_bottom_x1 , Area_Info_Get[buf[3]].line_bottom_x2,Area_Info_Get[buf[3]].line_top_y ,Area_Info_Get[buf[3]].line_second_y ,\
                                Area_Info_Get[buf[3]].line_third_y , Area_Info_Get[buf[3]].line_bottom_y );
                                
                                Area_Info_Get[buf[3]].recive_flag = 1;
                            }
                            else if(buf_temp[0] == 0xff && buf_temp[1] == 0xcc) //栈板检测参考线信息
                            {
                                printf("receive cam channel = %d\n", ch);
                                //只支持CAM1
                                if(ch == 0)
...
void pd_Alarm_Out_test(void *pvArg)
{
    //static uint8_t lock = 0;
    static uint16_t cnt = 20;
    uint8_t flag = 0;
    static PD_ALARM_TYPE lmode;
    ...
    if(( lmode && ALARM_OUTPUT_EN[stAlarmNotice.s32Chn]))
        flag = 1;
    ...
    if( flag )
    {
        ...
        {
            printf("alarm en\n");
            lmode = PD_ALARM_TYPE_RED;
            pd_Alarm_Out_Enable(lmode);
            alarm_out_lock = 1;
        }
        cnt = 20;
        //延时
        alarm_out_flag = stPalletDetectParams.u32TurnoffTime * 5 - 3;
    }
    // else 
    // {
    //     cnt --;
    //     if(alarm_out_lock && cnt == 0)
    //     {
...
void pd_Alarm_Out(void *pvArg)
{
    ...
    //printf("Interval:%d , Green:%d, Yellow:%d, Red:%d",pstPdsParam->s32PdAlarmOutInterval,pstPdsParam->bPdAlarmOutGreen, pstPdsParam->bPdAlarmOutYellow,pstPdsParam->bPdAlarmOutRed  );
    ...
    {
        lmode = stAlarmNotice.enMode;
        if (BOARD_IsCustomer(BOARD_C_ADA32V2_FTC))  //直接进入报警？pjw
        {
            pd_Alarm_Out_Enable(lmode);
        }
    }
...
sint32 pd_Alarm_Post(PD_ALARM_NOTICE_S stAlarmNotice)
{
  ...
  thpool_add_work(m_stPdInfo.thpool, pd_Alarm_Out_test,&stAlarmNoticeLocal);
...
void * pd_alg_Body(void *pvArg)
{
    ...
    // opticalflowanalyzeralg::EVehicleStateCode bResult;   //光流 result
    opticalflowanalyzeralg::STPalletDetectResult stPalletDetectResult;
    sint32 sFrameCount = 0;
...
#endif
        memset(&stPalletDetectResult, 0x00, sizeof(opticalflowanalyzeralg::STPalletDetectResult));
...
// #endif
        ...
        // s32Ret = cFlowAnalyzer->VehicleStateAnalysis(dstData[index], dstData[index], bResult, 0.5);
        // if(sFrameCount % 8)
        // {
        //     sFrameCount++;
        //     continue;
        ...
        //     sFrameCount++;
        // }
        // printf("VehicleStateAnalysis pre FrameHandle count = %d\n", sFrameCount);

        
        clock_gettime(CLOCK_MONOTONIC, &tvBegin);
        s32Ret = cPalletAnalyzer.PalletStateAnalysis((uint8_t*)dstData[index], stPalletDetectResult);
        clock_gettime(CLOCK_MONOTONIC, &tvEnd);
        long long int u32DecodeTimeMs = ((tvEnd.tv_sec*1000 + tvEnd.tv_nsec/1000000) - (tvBegin.tv_sec*1000 + tvBegin.tv_nsec/1000000));
        if(stPalletDetectResult.bDetectPallet)
        {
            print_level(SV_INFO, "decode time: %lld ms\n", u32DecodeTimeMs);
        }

        // if(stPalletDetectResult.u32NumofObj)
        // {
        //     std::cout<<"u32NumofObj:" << stPalletDetectResult.u32NumofObj << std::endl;
        // }
        
        // s32Ret = cPalletAnalyzer.testAlg((uint8_t*)dstData[index], stPalletDetectResult, 1);
        // printf("VehicleStateAnalysis after s32Ret = %d\n", s32Ret);
        if (0 != s32Ret)
        {
            print_level(SV_ERROR, "VehicleStateAnalysis failed. [err=%d]\n", s32Ret);
            ALG_Calculate_unLock();
            sleep_ms(10);
            continue;
        }
        ...
        // if(stPalletDetectResult.u32NumofObj)
        // {
        //     std::cout<<"bAlarmLevel:" << stPalletDetectResult.eAlarmLevel << std::endl; 
        //     std::cout<<"NumofObj:" << stPalletDetectResult.u32NumofObj << std::endl;
        //     for(i = 0; i < stPalletDetectResult.u32NumofObj; i++)
        //     {
        //         std::cout<<"stPalletDetectInfo "<< i << " cls:" << stPalletDetectResult.stPalletDetectInfo[i].cls << std::endl; 
        //         std::cout<<"stPalletDetectInfo "<< i << " fConfidence:" << stPalletDetectResult.stPalletDetectInfo[i].fConfidence << std::endl; 
        //         std::cout<<"stPalletDetectInfo "<< i << " fX1:" << stPalletDetectResult.stPalletDetectInfo[i].fX1 << std::endl;
        //         std::cout<<"stPalletDetectInfo "<< i << " fY1:" << stPalletDetectResult.stPalletDetectInfo[i].fY1 << std::endl;
        //         std::cout<<"stPalletDetectInfo "<< i << " fX2:" << stPalletDetectResult.stPalletDetectInfo[i].fX2 << std::endl;
        //         std::cout<<"stPalletDetectInfo "<< i << " fY2:" << stPalletDetectResult.stPalletDetectInfo[i].fY2 << std::endl;
        //     }
        // }
        // sFrameCount++;
#if 1
...
skip_alg:  
        ...
        { 
            ...
            if(stPalletDetectResult.stPalletDetectInfo[i].cls  == opticalflowanalyzeralg::E_CLS_PERSON)
            {
                 if(DETECT_MODE == 0x02) //只检车
                     continue;
                u8person_cnt++;
         
            }
            else if(stPalletDetectResult.stPalletDetectInfo[i].cls == opticalflowanalyzeralg::E_CLS_CAR)
            {
                 if(DETECT_MODE == 0x01) //只检人
                     continue;
                u8car_cnt++;
            }
            else if(stPalletDetectResult.stPalletDetectInfo[i].cls == opticalflowanalyzeralg::E_CLS_PALLET)
            {
                u8pallet_cnt++;
            }   


            ALG_RANGE_LIMIT(stPalletDetectResult.stPalletDetectInfo[i].fX1, 0.0, 1.0);
            ALG_RANGE_LIMIT(stPalletDetectResult.stPalletDetectInfo[i].fY1, 0.0, 1.0);
            ALG_RANGE_LIMIT(stPalletDetectResult.stPalletDetectInfo[i].fX2, 0.0, 1.0);
            ALG_RANGE_LIMIT(stPalletDetectResult.stPalletDetectInfo[i].fY2, 0.0, 1.0);


            Algo_Result_f stResults_f;
            /* 添加到串口数据中 */
            stResults_f.fx1 = stPalletDetectResult.stPalletDetectInfo[i].fX1 ;
            stResults_f.fy1 = stPalletDetectResult.stPalletDetectInfo[i].fY1 ;
            stResults_f.fx2 = stPalletDetectResult.stPalletDetectInfo[i].fX2 ;
            stResults_f.fy2 = stPalletDetectResult.stPalletDetectInfo[i].fY2 ;

            //转回为分辨率
            stAlgResult[u8LenAlgData].x1 = (short)(stResults_f.fx1 * 1024);
            stAlgResult[u8LenAlgData].y1 = (short)(stResults_f.fy1 * 600);
            stAlgResult[u8LenAlgData].x2 = (short)(stResults_f.fx2 * 1024);
            stAlgResult[u8LenAlgData].y2 = (short)(stResults_f.fy2 * 600);

           // s32AlarmMode = area_detect(stAlgResult[i],Area_Info_Get[SOURCE_ID]); //获取报警程度，旧
            //直接获取报警返回值
            s32AlarmMode = stPalletDetectResult.stPalletDetectInfo[i].eAlarmLevel;
//            printf("alarm:%d\n",s32AlarmMode);
            ...
            stAlgResult[u8LenAlgData].y1 = stAlgResult[u8LenAlgData].y1 +  (stAlgResult[u8LenAlgData].y2 - stAlgResult[u8LenAlgData].y1)/2;
            stAlgResult[u8LenAlgData].y2 = stAlgResult[u8LenAlgData].y1;

   // if(BsdRange_TYPE == 0)
    //{

        if(DisplayMode == DISPLAY_SINGLE){
            if ((stAlgResult[u8LenAlgData].x2 - stAlgResult[u8LenAlgData].x1 < 30) )
            {
                stAlgResult[u8LenAlgData].x2 = stAlgResult[u8LenAlgData].x1 + 30;
            }

        }
        else
        {
            if(DisplayMode == DISPLAY_QUAD1 || DisplayMode == DISPLAY_QUAD3 || DisplayMode == DISPLAY_DOUBLE1)
            {
                if (((stAlgResult[u8LenAlgData].x2 - stAlgResult[u8LenAlgData].x1 )< 30) )
                {
                    if((stAlgResult[u8LenAlgData].x2 + 30)>=510)
                    {
                        stAlgResult[u8LenAlgData].x2  = 510;
                        stAlgResult[u8LenAlgData].x1 = 510 -30;
                    }
                    else
                    {
                    stAlgResult[u8LenAlgData].x2 = stAlgResult[u8LenAlgData].x1 + 30;
            ...
            // if(((stAlgResult[u8LenAlgData].y2 - stAlgResult[u8LenAlgData].y1) < 15))
            // {
            //     stAlgResult[u8LenAlgData].y2 = stAlgResult[u8LenAlgData].y1 + 15;
            // }
        }

        //printf("[%d]x1:%d,x2:%d,y1:%d,y2:%d\n\r",SOURCE_ID,stAlgResult[u8LenAlgData].x1, stAlgResult[u8LenAlgData].x2, stAlgResult[u8LenAlgData].y1,stAlgResult[u8LenAlgData].y2);
    //}
...
sh: can't create /sys/class/gpio_stonkam/RGB/rgbstate: nonexistent directory***********/
         ...
         //s32AlarmMode = PD_ALARM_TYPE_NULL;
            ...
            //报警等级
            if(stPalletDetectResult.stPalletDetectInfo[i].cls  == opticalflowanalyzeralg::E_CLS_PERSON && stAlgResult[u8LenAlgData].alarm_type == opticalflowanalyzeralg::E_ALARM_RANGE_0)
            {   
                u8person_cnt--;
            }

            if(stPalletDetectResult.stPalletDetectInfo[i].cls  == opticalflowanalyzeralg::E_CLS_CAR  && stAlgResult[u8LenAlgData].alarm_type == opticalflowanalyzeralg::E_ALARM_RANGE_0)
            {   
                u8car_cnt--;
            }
            ...
            if(pstPdInfo->bSkipPerson[s32CurChn] && stPalletDetectResult.stPalletDetectInfo[i].cls == pdsa32::E_CLS_PERSON)
            {
                continue;
            }
            
            if(pstPdInfo->bSkipCar[s32CurChn] && stPalletDetectResult.stPalletDetectInfo[i].cls == pdsa32::E_CLS_CAR)
            {
                continue;
            }

            if(!bDetection && stPalletDetectResult.stPalletDetectInfo[i].cls == pdsa32::E_CLS_CAR)
            {
                continue;
            }
#if PD_OVERTAKE_ALARM
...
//串口组包
        ...
        if(stPalletDetectResult.bLampSwitch)
        {
            pc_flag |= 0x10;
        }
        ...
        uint8_t send_cnt = 0;
        // if(bResult != opticalflowanalyzeralg::E_STATE_STATIONARY)
        if(DETECT_AREA_EN)
        {
            for(uint8_t i = 0; i < u8LenAlgData; i++)
            {
                if(stAlgResult[i].alarm_type != PD_ALARM_TYPE_NULL)
                {
                    memcpy(&u8SerialData[5+(sizeof(Algo_Result))*send_cnt], &stAlgResult[i], (sizeof(Algo_Result))); 
                    send_cnt ++ ;
                }
            }
            memcpy(&u8SerialData[4], &send_cnt, 1);

            u16Crc16 = crc16_Gen(&u8SerialData[2], sizeof(Algo_Result) * send_cnt + 3); //加一位表示当前通道
    
            memcpy(&u8SerialData[sizeof(Algo_Result) * send_cnt + 5], &u16Crc16, 2);  //向后移一位


    
            if(SOURCE_ID < 4)
            {
                s32WriteLen = write(m_stPdInfo.s32SerialFd[0], u8SerialData, sizeof(Algo_Result) * send_cnt + 7);
                //printf("detect num[%d]:%d,send:%d  \n",SOURCE_ID ,u8LenAlgData,send_cnt );
            }
        }
        else
        {
            ...
            if(SOURCE_ID < 4)
            {
                ...
                //printf("detect num[%d]:%d,send:%d \n",SOURCE_ID ,stPdResult.u32Nums,u8LenAlgData );
            }
        }
...
skip_alg:  
        ...
        { 
            ...
            else if(stPdResult.stResults[i].classes == pdsa32::E_CLS_CAR)
            {
                 if(DETECT_MODE == 0x01) //只检人
                     continue;
                u8car_cnt++;
...
sh: can't create /sys/class/gpio_stonkam/RGB/rgbstate: nonexistent directory***********/
         ...
         //s32AlarmMode = PD_ALARM_TYPE_NULL;
            ...
            if(stPdResult.stResults[i].classes == pdsa32::E_CLS_PERSON && stAlgResult[u8LenAlgData].alarm_type == PD_ALARM_TYPE_NULL)
            {   
                u8person_cnt--;
            }

            if(stPdResult.stResults[i].classes == pdsa32::E_CLS_CAR  && stAlgResult[u8LenAlgData].alarm_type == PD_ALARM_TYPE_NULL)
            {   
                u8car_cnt--;
            }
...
sint32 PD_Init(PD_CFG_PARAM_S *pstInitParam)
{
   ...
   // Area_Info_init();
    ...
    s32Ret = pthread_create(&u32Tid_v4l2Save, &attr, mpp_vpss_FrameSave,NULL);//v4l2抓图
    ...
    // [1] 光流算法初始化
    // 模型路径
    // opticalflowanalyzeralg::STAlgModelPath stModelsPath = {nullptr};
    // const char* modelpath = PD_MODEL_FLOWNET;
    // stModelsPath.pModelPath = modelpath;    // 光流模型
    
    stPalletDetectParams.stOpticalFlowModelPath = stOFModelsPath;
    stPalletDetectParams.stPalletDetectModelPath = stPDModelsPath;

    s32Ret = cPalletAnalyzer.ImportParams(PALLET_CONFIG, stPalletDetectParams);
    printf("stOpticalFlowModelPath = %s\n", stPalletDetectParams.stOpticalFlowModelPath.pModelPath);
    printf("stPalletDetectModelPath = %s\n", stPalletDetectParams.stPalletDetectModelPath.pModelPath);
    if(s32Ret != SV_SUCCESS)
    {
...
toPcModel:
                ...
                {
                    printf("set apcsPdsAlgRgbPC type\n");
