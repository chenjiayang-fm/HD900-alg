...
#define version "1126-01-V1.0-20260901"
...
typedef struct tagPdAlarmNotice_S
{
    ...
    uint8           u8DisplayMode;          /* 当前帧在画面中的显示位置 */
} PD_ALARM_NOTICE_S;
...
static SV_BOOL pd_IsCh4InQuadView(const PD_ALARM_NOTICE_S *pstAlarmNotice)
{
    return (SV_BOOL)(pstAlarmNotice->s32Chn == 3
        && pstAlarmNotice->u8DisplayMode >= DISPLAY_QUAD1
        && pstAlarmNotice->u8DisplayMode <= DISPLAY_QUAD4);
}
...
void sendcmd2sonix(int port,int CH,int CMD) //CMD_RESET_IFRAME = 0,      CMD_AREA_INFO=1 ,
{
...
sint32 mpp_vpss_Vdec()
{
   ...
   // sint32 s32Fd = m_stVpssAlgFrame.s32AlgChnFd;
    ...
    // uint64_t au64VdecRgaTimeNs[8] = {0};
    // uint32_t au32VdecRgaFrameCnt[8] = {0};
    ...
    else
    {
        ...
        // struct timespec stPerfStart = {0, 0};
        // struct timespec stPerfEnd = {0, 0};
        // clock_gettime(CLOCK_MONOTONIC_RAW, &stPerfStart);

        s32Ret =  RK_MPI_SYS_SendMediaBuffer(RK_ID_VDEC, VDEC_CH , m_mb);
        // SV_BOOL bPerfFrameValid = (s32Ret == RK_SUCCESS) ? SV_TRUE : SV_FALSE;
        if(s32Ret != RK_SUCCESS)
        {
        ...
        MEDIA_BUFFER RGA_m_mb = RK_MPI_SYS_GetMediaBuffer(RK_ID_RGA, VDEC_CH, 10000); //解码数据直接送到RGA
            ...
            // clock_gettime(CLOCK_MONOTONIC_RAW, &stPerfEnd);
            // if (bPerfFrameValid && VDEC_CH < 8)
            // {
            //     uint64_t u64PerfStartNs = (uint64_t)stPerfStart.tv_sec * 1000000000ULL
            //                             + (uint64_t)stPerfStart.tv_nsec;
            //     uint64_t u64PerfEndNs = (uint64_t)stPerfEnd.tv_sec * 1000000000ULL
            //                           + (uint64_t)stPerfEnd.tv_nsec;
            //     uint64_t u64VdecRgaTimeNs = u64PerfEndNs - u64PerfStartNs;
            //     uint8_t u8FrameSeq = MiddleData[index_vdec][7];
            //     SV_BOOL bIFrame = (MiddleData[index_vdec][6] == 0) ? SV_TRUE : SV_FALSE;

            //     au64VdecRgaTimeNs[VDEC_CH] += u64VdecRgaTimeNs;
            //     au32VdecRgaFrameCnt[VDEC_CH]++;

            //     if (bIFrame)
            //     {
            //         printf("[PERF][I][CH=%u][VDEC_CH=%u][SEQ=%u] vdec_rga=%.3fms\n",
            //                (unsigned int)Getframe_CH, (unsigned int)VDEC_CH,
            //                (unsigned int)u8FrameSeq,
            //                (double)u64VdecRgaTimeNs / 1000000.0);
            //     }

            //     if (au32VdecRgaFrameCnt[VDEC_CH] >= 30)
            //     {
            //         printf("[PERF][AVG30][CH=%u][VDEC_CH=%u] vdec_rga=%.3fms\n",
            //                (unsigned int)Getframe_CH, (unsigned int)VDEC_CH,
            //                (double)au64VdecRgaTimeNs[VDEC_CH]
            //                    / (double)au32VdecRgaFrameCnt[VDEC_CH] / 1000000.0);
            //         au64VdecRgaTimeNs[VDEC_CH] = 0;
            //         au32VdecRgaFrameCnt[VDEC_CH] = 0;
            //     }
            // }
...
sint32 pd_Alarm_Out_Enable(PD_ALARM_TYPE enAlarmType)
{
    ...
    /* Only red alarms drive the physical AlarmOut signal. */
    if (enAlarmType != PD_ALARM_TYPE_RED)
    {
        return pd_Alarm_Out_Reset();
    }
            ...
            {     
                if(flag == 0)
                {
                    ...
                    if(buf_temp[0] == 0xff && buf_temp[1] == 0xbb) //检测区域信息
                    {
                        ...
                        if(u16Crc == ((buf_temp[(buf_temp[2]-1)]<< 8) + buf_temp[buf_temp[2]-2]) ){  //crc校验通过
                ...
                else
                {
                    ...
                    printf("crc: %x - %x\n", u16Crc,((buf[(buf[2]-1)]<< 8) + buf[buf[2]-2]));
                        ...
                        if(u16Crc == ((buf[(buf[2]-1)]<< 8) + buf[buf[2]-2]) ){  //crc校验通过
...
void pd_Alarm_Out_test(void *pvArg)
{
    ...
    if((lmode == PD_ALARM_TYPE_RED)
       && (stAlarmNotice.s32Chn >= 0 && stAlarmNotice.s32Chn < 4)
       && ALARM_OUTPUT_EN[stAlarmNotice.s32Chn]
       && !pd_IsCh4InQuadView(&stAlarmNotice))
        flag = 1;
...
void pd_Alarm_Out(void *pvArg)
{
    ...
    if(pd_IsCh4InQuadView(&stAlarmNotice))
    {
        goto skip_alg_twice;
    }
...
sint32 pd_Alarm_Post(PD_ALARM_NOTICE_S stAlarmNotice)
{
    ...
    return SV_SUCCESS;
#endif
    /* AlarmOut is short and must consume the current frame's channel data. */
    pd_Alarm_Out_test(&stAlarmNotice);
    ...
    stAlarmNoticeLocal.u8DisplayMode = stAlarmNotice.u8DisplayMode;
...
//串口组包
       ...
       stAlarmNotic.u8DisplayMode = DisplayMode;
...
sint32 pd_PostShelterResult()
{
    ...
    PD_ALARM_NOTICE_S stPdAlarmNotice = {0};
