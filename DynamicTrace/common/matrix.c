...
M_memorylinknode matrix_headnode = {NULL,NULL};


Matrix* M_applymemory(){
	M_memorylinknode *subnode = (M_memorylinknode*)malloc(sizeof(M_memorylinknode));
	subnode->matrix = (Matrix*)malloc(sizeof(Matrix));
	subnode ->next =  matrix_headnode.next;
	matrix_headnode.next = subnode;
	return subnode->matrix;
}
//表示释放自第num以及第num个之后的链表元素(num从0开始计数)
int M_releasememory(int num){
	M_memorylinknode *prior,*posterior;
	posterior = matrix_headnode.next;
	prior = &matrix_headnode;
	int i=0,j=0;
	while(i<num && posterior!=NULL){
		prior = posterior;
...
int M_lengthmemory(){
	int i=0;
	M_memorylinknode *subnode = matrix_headnode.next;
	while(subnode!=NULL){
	...
	for (i=0;i<_mat->row;i++){
		for(j=0;j<_mat->column;j++){
			//printf(PRECISION,_mat->data[i*(_mat->column)+j]);
 			printf("%.4f |",_mat->data[i*(_mat->column)+j]);
 		}
 		printf("\n");
	}
	return 0;
...
Uptri_struct* M_Uptri_4inv (Matrix* _mat_source){/*Upper_triangular_transformation_for_Inverse 
	...
	Etrans_struct *_free_top, *_free_temp = NULL;
	Etrans_struct _free_head;
	_free_top = &_free_head;
	...
	for(i=0;i<_mat->column;i++){
		for(j=i+1;j<_mat->row;j++){
			...
			_free_top->next_E_trans = _Etrans_temp;
			_free_top = _Etrans_temp;
...
//	printf(">>Uptri(Matrix_%x)=\n",_mat_source);
//	printf("\tMatrix_%x * Matrix_%x\n",_mat,trans_mat);
	_free_top->next_E_trans = NULL;
	_free_top = _free_head.next_E_trans;
	while(_free_top!=NULL){
		_free_temp = _free_top;
		_free_top = _free_top->next_E_trans;
		free(_free_temp);
	}
...
Matrix* M_numul(Matrix* _mat,MATRIX_TYPE _num){/*Matrix Multiply
	矩阵数乘 
	_mat_result = _mat_left*_mat_right */
	int row = _mat->row,column = _mat->column,size=(_mat->row)*(_mat->column);
	Matrix *new_mat = M_applymemory();
	MATRIX_TYPE* data = (MATRIX_TYPE*)malloc(size*sizeof(MATRIX_TYPE));
	MATRIX_TYPE* data1 = _mat->data;
...
Matrix* M_Nones(int row,int column){
	//生成全空矩阵
	Matrix *None_mat = M_applymemory();
	None_mat->data = (MATRIX_TYPE*)malloc((row*column)*sizeof(MATRIX_TYPE));
	None_mat->row = row;
	None_mat->column = column;
	return None_mat;
}

Matrix* M_Blank(int row, int column){
	//生成没有数据的矩阵
	Matrix *Blank_mat = M_applymemory();
	Blank_mat->row = row;
	Blank_mat->column = column;
	return Blank_mat;
}
...
Matrix* M_find(Matrix* _mat, MATRIX_TYPE value){
	...
	Matrix* mat_result = M_applymemory();
	mat_result->row = num;
	mat_result->column = 1;
...
Matrix* M_sum(Matrix* _mat,int dim){
	...
	if(dim==0){
		MATRIX_TYPE* data = (MATRIX_TYPE*)malloc(column*sizeof(MATRIX_TYPE));
		for(i=0;i<column;i++){
			data[i] = 0;
			for(j=0;j<row;j++){
				data[i] += _mat->data[j*column+i];
			}
		}
		mat_result -> data = data;
		mat_result -> row = 1;
		...
		MATRIX_TYPE* data = (MATRIX_TYPE*)malloc(row*sizeof(MATRIX_TYPE));
		for(i=0;i<row;i++){
			data[i] = 0;
			for(j=0;j<column;j++){
				data[i] += _mat->data[i*column+j];
			}
			mat_result -> data = data;
			mat_result -> row = row;
			mat_result -> column = 1;
...
Matrix* M_logic_equal(Matrix* _mat, MATRIX_TYPE value){
	int size_mat = (_mat->row)*(_mat->column), i;
	Matrix* mat_logic = M_applymemory();
