...
RK_S32 RK_MPI_VI_EnableChn(VI_PIPE ViPipe, VI_CHN ViChn) {
  ...
  RK_S8 s8RetryCnt = 3;
  while (s8RetryCnt > 0) {
    g_vi_chns[ViChn].rkmedia_flow = easymedia::REFLECTOR(Flow)::Create<easymedia::Flow>(flow_name.c_str(), flow_param.c_str());
    printf("%s: VI[%d]  create rkmedia_flow, flow_name=%s, flow_param=%s\n", __func__, ViChn, flow_name.c_str(), flow_param.c_str());
...
RK_S32 RK_MPI_VI_DisableChn(VI_PIPE ViPipe, VI_CHN ViChn) {
  ...
  printf("%s: Disable VI[%d:%d]:%s, %dx%d Start...\n", __func__, ViPipe,
               ViChn, g_vi_chns[ViChn].vi_attr.attr.pcVideoNode,
               g_vi_chns[ViChn].vi_attr.attr.u32Width,
               g_vi_chns[ViChn].vi_attr.attr.u32Height);
  ...
  printf("%s: VI[%d] before rkmedia_flow reset\n", __func__, ViChn);
  usleep(100*1000);
  g_vi_chns[ViChn].rkmedia_flow.reset();
  printf("%s: VI[%d] after rkmedia_flow reset\n", __func__, ViChn);
  
  if (!g_vi_chns[ViChn].buffer_list.empty()) {
    printf("%s: clear buffer list again...\n", __func__);
    RkmediaChnClearBuffer(&g_vi_chns[ViChn]);
  }
  g_vi_mtx.unlock();

  printf("%s: Disable VI[%d:%d]:%s, %dx%d End...\n", __func__, ViPipe,
               ViChn, g_vi_chns[ViChn].vi_attr.attr.pcVideoNode,
               g_vi_chns[ViChn].vi_attr.attr.u32Width,
               g_vi_chns[ViChn].vi_attr.attr.u32Height);
...
RK_S32 RK_MPI_VDEC_CreateChn(VDEC_CHN VdChn, const VDEC_CHN_ATTR_S *pstAttr) {
  ...
  if (pipe2(g_vdec_chns[VdChn].wake_fd, O_CLOEXEC) == -1) {
    g_vdec_chns[VdChn].wake_fd[0] = 0;
    g_vdec_chns[VdChn].wake_fd[1] = 0;
    RKMEDIA_LOGW("Create pipe failed!\n");
  }
...
RK_S32 RK_MPI_VDEC_DestroyChn(VDEC_CHN VdChn) {
  ...
  g_vdec_chns[VdChn].status = CHN_STATUS_CLOSED;
  if (g_vdec_chns[VdChn].wake_fd[0] > 0) {
    close(g_vdec_chns[VdChn].wake_fd[0]);
    g_vdec_chns[VdChn].wake_fd[0] = 0;
  }
  if (g_vdec_chns[VdChn].wake_fd[1] > 0) {
    close(g_vdec_chns[VdChn].wake_fd[1]);
    g_vdec_chns[VdChn].wake_fd[1] = 0;
  }
  g_vdec_mtx.unlock();
...
RK_S32 RK_MPI_VDEC_GetFd(VDEC_CHN VdChn) {
  ...
  int rcv_fd = 0;
  g_vdec_mtx.lock();
  if (g_vdec_chns[VdChn].status < CHN_STATUS_OPEN) {
    g_vdec_mtx.unlock();
    return -RK_ERR_VENC_NOTREADY;
  }
  rcv_fd = g_vdec_chns[VdChn].wake_fd[0];
  g_vdec_mtx.unlock();
...
RK_S32 RK_MPI_VMIX_CreateDev(VMIX_DEV VmDev, VMIX_DEV_INFO_S *pstDevInfo) {
  ...
  RGA_FLIP_E enFlip = RGA_FLIP_NULL;
  RK_U16 u16Rotaion = 0;
