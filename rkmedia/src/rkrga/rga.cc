...
int RgaFilter::IoCtrl(unsigned long int request, ...) {
  ...
  case S_RGA_CFG: {
    ...
    FlipEnum new_filp = (FlipEnum)new_rga_cfg->flip;
    int max_width, max_height;
    int max_dst_width, max_dst_height;
    if (new_rotation == 90 || new_rotation == 270) {
      max_width = src_max_height;
      max_height = src_max_width;
      max_dst_width  = dst_max_height;
      max_dst_height = dst_max_width;
    } else {
      max_width = src_max_width;
      max_height = src_max_height;
      max_dst_width  = dst_max_width;
      max_dst_height = dst_max_height;
    }
    /*RKMEDIA_LOGI(
          "IoCtrl: max_width max_height max_dst_width max_dst_height:<%d,%d,%d,%d> \n",
          max_width, max_height, max_dst_width, max_dst_height);*/
    if (rga_rect_check(new_src_rect, max_width, max_height) /*||
        rga_rect_check(new_dst_rect, max_dst_width, max_dst_height)*/) {
      RKMEDIA_LOGE(
          "IoCtrl: Invalid srcRect:<%d,%d,%d,%d> or dstRect:<%d,%d,%d,%d>,  max_src<%d, %d> max_dst<%d, %d>\n",
    ...
    if ((new_rotation != 0) && (new_rotation != 90) && (new_rotation != 180) &&
     ...
     if ((new_filp != FLIP_NULL) && (new_filp != FLIP_H) && (new_filp != FLIP_V) && 
     	(new_filp != FLIP_HV)) {
      RKMEDIA_LOGE("IoCtrl: Invalid filp:%d\n", new_filp);
      ret = -1;
      break;
    }
  ...
  case G_RGA_CFG: {
	...
	FlipEnum *rd_filp = (FlipEnum *)&(rd_rga_cfg->flip);
    param_mtx.lock();
