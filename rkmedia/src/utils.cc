...
static void read_log_level() {
  ...
  int log_level;
  int fd = open(LOG_FILE_PATH, O_RDONLY | O_CLOEXEC);
  if (fd == -1)
    return;
...
_API void LOG_INIT() {
  ...
  int log_file_fd = open(LOG_FILE_PATH, O_RDWR | O_CREAT | O_TRUNC | O_CLOEXEC);
  if (log_file_fd != -1) {
...
bool DumpToFile(std::string path, const char *ptr, size_t len) {
  int fd = open(path.c_str(), O_CREAT | O_WRONLY | O_FSYNC | O_CLOEXEC);
  if (fd < 0) {
