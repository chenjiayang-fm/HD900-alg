...
typedef int32_t (*ALG_CHECK_CALLBACK)(uint8_t* au8SourceAddr, uint8_t* au8ResultAddr);
    ...
    struct STOverTakeParam
    {
        ...
        bool bOpticalFlow; 
        int s32FrameInterval;
        float fReferenceRate;
        bool bOpticalFreeShake;

        // 默认值
        STOverTakeParam():eViewType(E_CAM_VIEW_FORWARD),s32Delay(5),fChangingTHR(-0.0008),fMinX(0),fMaxX(1),fMinY(0),fMaxY(1),fdistance(0.05),s32frameNum(7),fspeed(0),bOpticalFlow(false),s32FrameInterval(0),fReferenceRate(0.8),bOpticalFreeShake(true){}

        // 用户指定
        STOverTakeParam(EAlgCamViewType eType, int32_t s32d, float fth, float fx1, float fx2, float fy1, float fy2,float fdistance_in,int fframe_in,float fspeed_in, bool useOpticalFLow, int frameInterval, int32_t ReferenceFrameSubtractNum, bool OpticalFreeShake):\
                            eViewType(eType), s32Delay(s32d), fChangingTHR(fth), fMinX(fx1), fMaxX(fx2), fMinY(fy1), fMaxY(fy2), fdistance(fdistance_in),\
                            s32frameNum(fframe_in), fspeed(fspeed_in),bOpticalFlow(useOpticalFLow),s32FrameInterval(frameInterval),fReferenceRate(ReferenceFrameSubtractNum),bOpticalFreeShake(OpticalFreeShake){}
    };
 ...
 ***********************/
    ...
    public:
        ...
        CPdsAlg(EAlgType type, pdsa32::EAlgCamViewType eCamViewType, uint32_t u32OpPointNumLim = 100, bool bOpflowPrint = false,bool bDebug=false);
         ...
         * 应用层动态设置当前的总上下车人数
         * 输入:
         *     s32HeadInNum, 总上车人数信息  s32HeadOutNum,总下车人数信息
         * 
         * 输出:
         *     无
         *     
         ...
         *     版本字符串指针           
         ********************************************************************************/
        int32_t AlgAPCDynamicSetting(int32_t s32HeadInNum, int32_t s32HeadOutNum,uint32_t u32Channel = 0);
